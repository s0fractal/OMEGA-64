...
Deno.test("Stage 31: Economic Swap Protocol (Cross-Chain P2P Transfer)", async () => {
  ...
  assertEquals(senderEnergy < 800, true, "Sender must have < 800 Energy (1000 initial - 200 sent - 10 gas tax)");
  assertEquals(receiverEnergy > 295 && receiverEnergy < 305, true, "Receiver must have ~300 Energy (100 initial + 200 received - ~1 metabolic tax)");

  // Test Resonance Transfer next tick
  senderScript.fill(0);
  senderScript[0] = RISC.OP_SET; senderScript[1] = 0; senderScript[2] = SYS.TRANSFER; 
  senderScript[3] = RISC.OP_SET; senderScript[4] = 1; senderScript[5] = receiverIdx; 
  senderScript[6] = RISC.OP_SET; senderScript[7] = 2; senderScript[8] = 1; // 1 = Resonance
  senderScript[9] = RISC.OP_SET; senderScript[10] = 3; senderScript[11] = 25; // Send 25 Resonance
  senderScript[12] = RISC.OP_SYSCALL; senderScript[13] = 0; 
  STATE_MATRIX.setInstructions(senderIdx, senderScript);
  ...
  assertEquals(senderResonance, 21, "Sender Resonance should be 21 (50 - 2 decay - 25 sent - 2 decay)");
  assertEquals(receiverResonance, 31, "Receiver Resonance should be 31 (10 - 2 decay + 25 received - 2 decay)");
