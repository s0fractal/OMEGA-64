...
async function runGeneticEvolutionTests() {
    ...
    const execute_atom = instantiated.instance.exports.execute_atom as (idx: number) => void;
    ...
    const readXs = new Int16Array(STATE_MATRIX.buffer, 8000000 + 40400000, 100000);
    const readYs = new Int16Array(STATE_MATRIX.buffer, 8000000 + 40600000, 100000);
    const readEnergy = new Int32Array(STATE_MATRIX.buffer, 8000000 + 40800000, 100000);
    const readResonance = new Int32Array(STATE_MATRIX.buffer, 8000000 + 41200000, 100000);
    ...
    // Set Logic (Genome)
    const logicView = new BigUint64Array(STATE_MATRIX.buffer, 8000000 + 2400000, 100000); // LOGIC_OFFSET
    ...
    const spawnBuffer = new DataView(STATE_MATRIX.buffer, 8000000 + 19600000 + 8, 1024 * 16);
