...
async function testCrystallineNetwork() {
    ...
    if (finalCharge > 200) {
        ...
        console.error(`❌ [TEST] Signal failed to reach destination. Final Charge: ${finalCharge}`);
        Deno.exit(1);
    }
    ...
    STATE_MATRIX.seedAtom(testIdx, 1n, (endX * 10) + 5, (startY * 10) + 5, 1000, 0, genome, riscCode);
    ...
    console.log(`   [TEST] Atom status: PC=${pcValue}, Energy=${energyValue}, Reg1=${reg1Value}`);
    ...
    if (reg1Value > 200) {
        console.log("✅ [TEST] OP_PLUG successfully read lattice charge into atom register.");
    } else {
