...
function makeSynStack(w0: number, w1: number, w2: number, tally = 0): Int32Array {
    const arr = new Int32Array(new SharedArrayBuffer(4 * 4));
    arr[0] = w0; arr[1] = w1; arr[2] = w2; arr[3] = tally;
    return arr;
}

function baseState(synStack: Int32Array, overrides: Record<string, unknown> = {}) {
    return {
        x: 500, y: 400,
...
Deno.test("Era 57: ISA.DECAY auto prunes weakest synapse slot", () => {
    ...
    assertGreater(result.resonanceDelta, 0, "DECAY gives pruning resonance bonus");
...
Deno.test("Era 57: Passive decay skipped when pulseId % 10 !== 0", () => {
    ...
    assertEquals(STATE_MATRIX.getSynapticValue(idx, 0), 50, "Weight unchanged on non-decay tick");
    STATE_MATRIX.setId(idx, 0n);
});
...
Deno.test("Era 57: If HEBB fired this tick, passive decay skipped (simulated)", () => {
    ...
    assertEquals(STATE_MATRIX.getSynapticValue(idx, 0), 50, "Weight stays 50 when HEBB protected");
    STATE_MATRIX.setId(idx, 0n);
});
