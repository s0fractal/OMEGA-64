...
async function runTest() {
    ...
    const spawnAtom = (idx: number, x: number, y: number, logicBytes: Uint8Array) => {
    ...
    const genomeA = new Uint8Array([0x46, 0xAA, 0xBB, 0xCC, 0x01, 0x00, 0x00, 0x00]);
    ...
    if (crystalsAfter > crystalsBefore) {
        console.log(`✅ Territory expanded! Crystals: ${crystalsBefore} → ${crystalsAfter} (+${crystalsAfter - crystalsBefore})`);
    } else {
        console.log(`ℹ️  Crystals: ${crystalsBefore} → ${crystalsAfter} (expansion may need more ticks)`);
    }
    ...
    const genomeB = new Uint8Array([0x46, 0xBB, 0x11, 0x22, 0x02, 0x00, 0x00, 0x00]);
    ...
    if (signalAfter < signalBefore) {
        ...
        if (typeAfter === 0) {
            console.log(`🏆 Colony A territory CONQUERED — cell reverted to neutral!`);
        }
    } else {
