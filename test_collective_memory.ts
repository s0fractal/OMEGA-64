...
function baseState(overrides: Record<string, unknown> = {}): Parameters<typeof LAMBDA_VM.execute>[3] {
    return {
        x: 500, y: 400,
...
Deno.test("Era 51: ISA.IMPRINT emits imprintRequest when resonance > 20", () => {
    ...
    assertEquals(result.imprintRequest!.pheroSnapshot, (100 << 8) | 3, "Snapshot should match pre-seeded pheromone");
...
Deno.test("Era 51: ISA.IMPRINT suppressed when resonance ≤ 20", () => {
    ...
    assertEquals(result.imprintRequest, undefined, "IMPRINT should not fire when resonance ≤ 20");
});
...
Deno.test("Era 51: applying imprintRequest writes correct 16-byte layout to hiveMemory", () => {
    ...
    const gx = Math.floor(x / 10); const gy = Math.floor(y / 10);
    ...
    assertEquals(readPulse, pulseId, "Pulse ID (tick) should be stored correctly");
});
...
Deno.test("Era 51: ISA.RECALL reads pheromone intensity field (p1=0) into register", () => {
    ...
    assertEquals(context[2], 120, "Register 0 should hold recalled phero intensity = 120");
...
Deno.test("Era 51: ISA.SENSE type 0x07 reads hive-memory intensity into register", () => {
    ...
    assertEquals(context[3], 200, "Register 1 should hold hive-memory intensity = 200");
});
