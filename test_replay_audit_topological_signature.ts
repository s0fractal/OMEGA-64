...
Deno.test("replay audit validates topological signature fields", async () => {
    await withTempLedger(async () => {
        ...
        if (!result.replayGreen) {
            throw new Error(`replay should be green, got failures: ${result.failures.join(",")}`);
        }
        if (result.checkedProjectionEvents !== 1) {
            throw new Error(`expected 1 checked projection event, got ${result.checkedProjectionEvents}`);
        }
        if (result.skippedProjectionEvents !== 0) {
            throw new Error(`expected 0 skipped projection events, got ${result.skippedProjectionEvents}`);
        }
    });
});
...
Deno.test("replay audit fails when projection hashes are tampered", async () => {
    await withTempLedger(async () => {
        ...
        if (!result.failures.some((f) => f.includes("projection mismatch"))) {
            throw new Error(`expected projection mismatch failure, got: ${result.failures.join(",")}`);
        }
    });
});
