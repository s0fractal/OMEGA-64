...
async function runSecretionOpcodeTests() {
    ...
    if (snapshot.atomRolePlasmid.architect !== 1) {
        console.error("❌ OP_COLLECTIVE mode 7 failed to increment Architect Plasmid stat.");
        Deno.exit(1);
    }
    ...
    console.log(`Pheromone Architect Stat: ${snapshot.atomRolePheromone.architect}`);
    ...
    const tick_glyph_transport = instantiated.instance.exports.tickGlyphTransport as (tick: number) => void;
    
    GLYPH_BUFFER.clear();
    
    // Set high signal in a cell (use multiple of 32 due to WASM sampling)
    const cellIdx = 512;
    Atomics.store(new Int32Array(STATE_MATRIX.buffer, 35200000 + 8000000, 140*80), cellIdx, 1000); // SIGNAL_GRID_OFF
    
    // Set high memory in a cell
    const memoryBase = cellIdx * 8;
    const memoryGrid = new Uint8Array(STATE_MATRIX.buffer, 36200000 + 8000000, 140*80*8);
