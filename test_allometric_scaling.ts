...
Deno.test({
  ...
  console.log(`Bonded Atom Executions: ${bondedR1}, Energy Cost: ${bondedCost}`);

  // Assert Computations
  // The exact amount depends on gas limits, but Free > Bonded strictly
  if (freeR1 <= bondedR1) {
    throw new Error("Computations were not allometrically scaled. Free: " + freeR1 + ", Bonded: " + bondedR1);
  }
  ...
  if (bondedCost >= freeCost) {
     throw new Error("Metabolism was not discounted for topological mass. Free Cost: " + freeCost + ", Bonded Cost: " + bondedCost);
  }
  }
});
