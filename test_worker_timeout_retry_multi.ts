...
const debugDelayMs = Number.parseInt(
  Deno.env.get("OMEGA_WORKER_DEBUG_DELAY_MS") ?? "20",
  10,
);
...
async function main() {
  ...
  try {
    ...
    if (stats.length !== 4) {
      throw new Error(
        `[TEST] Expected exactly four worker stats, got=${stats.length}`,
      );
    }
    ...
    for (const w of stats) {
      ...
      if (w.requests < 1) {
        throw new Error(
          `[TEST] Expected worker-${w.workerIndex} to receive requests.`,
        );
      }
      if (w.retryWaits < 1) {
        throw new Error(
          `[TEST] Expected worker-${w.workerIndex} to hit at least one retry wait window.`,
        );
      }
      if (w.failures !== 0) {
        throw new Error(
    ...
    if (totalRetries < 4) {
      throw new Error(
        `[TEST] Expected total retries >= 4 across workers, got=${totalRetries}`,
      );
    }
