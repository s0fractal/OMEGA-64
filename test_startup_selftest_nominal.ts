...
function assertNominalStartup(expectedWorkers: number): void {
    ...
    console.log(`   runtime workers=${runtimeWorkerCount} faultStats=${faults.length}`);

    if (!status.done) throw new Error("[TEST] Startup self-test did not complete.");
    if (status.fallbackActivated) throw new Error("[TEST] Startup fallback must not activate in nominal run.");
    if (status.lastBreachTick !== -1) {
        throw new Error(`[TEST] Expected no startup breach, got tick=${status.lastBreachTick}`);
    }
    if (runtimeWorkerCount !== expectedWorkers) {
        throw new Error(`[TEST] Expected runtime workers=${expectedWorkers}, got=${runtimeWorkerCount}`);
    }
    if (faults.length !== expectedWorkers) {
        throw new Error(`[TEST] Expected fault stats for ${expectedWorkers} workers, got=${faults.length}`);
    }
    if (STATE_MATRIX.getActiveIndices().length !== 0 || STATE_MATRIX.getId(0) !== 0n) {
...
async function main() {
    ...
    try {
        ...
        await PULSE.tick();
        if (STATE_MATRIX.getActiveIndices().length !== 0 || STATE_MATRIX.getId(0) !== 0n) {
            throw new Error("[TEST] Matrix must stay empty after first nominal tick.");
        }

        PULSE.stopWorkers();
        const afterStop = PULSE.getStartupSelfTestStatus();
        if (afterStop.done || afterStop.fallbackActivated || afterStop.lastBreachTick !== -1) {
            throw new Error("[TEST] stopWorkers() must reset startup self-test cold-start state.");
        }
