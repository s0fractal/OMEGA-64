...
const alpha = new Deno.Command("deno", {
    args: ["run", "--allow-read", "--allow-write", "--allow-net", "i.L43.core.LOOP.ts", "--port=8081"],
    stdout: "inherit",
    stderr: "inherit"
}).spawn();
...
const beta = new Deno.Command("deno", {
    args: ["run", "--allow-read", "--allow-write", "--allow-net", "i.L43.core.LOOP.ts", "--port=8082"],
    stdout: "inherit",
    stderr: "inherit"
}).spawn();
...
try {
    ...
    if (swarm.length >= 2) {
        ...
        if (ports.some((p: string) => p.includes("8081")) && ports.some((p: string) => p.includes("8082"))) {
