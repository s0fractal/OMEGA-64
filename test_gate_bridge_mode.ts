...
async function readBridgeAndLedger(): Promise<{ bridge: BridgeModeEvent; ledger: LedgerEvent }> {
    const raw = [];
    for await (const evt of LEDGER.readAllRaw()) {
        raw.push(evt);
    }

    const bridge = raw.find((x) => "event_type" in x && x.event_type === "BRIDGE_MODE_EVENT");
    if (!bridge) {
...
Deno.test("bridge GREEN allows canon-bound proposals", async () => {
    ...
    try {
        ...
        if (ledger.rejected_proposals.some((r) => r.proposal_id === "p_canon_green")) {
