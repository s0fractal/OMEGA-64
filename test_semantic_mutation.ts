...
Deno.test("Stage 29: Open Semantic Mutation via SYS_MUTATE", async () => {
  ...
  assertEquals(currentInst[2], SYS.MUTATE, "Initial instruction at offset 2 should be SYS.MUTATE (7)");
  ...
  assertEquals(currentInst[2], 0, "Instruction at offset 2 should have been mutated to 0 (OP_HALT)");
  ...
  assertEquals(remainingEnergy < 955 && remainingEnergy > 945, true, `Energy should reflect the high cost of SYS_MUTATE (~950). Got: ${remainingEnergy}`);
