...
async function runTest() {
    ...
    console.log("💎 Test 1: 8 atoms with same genome broadcast at one location...");

    const SHARED_GENOME = new Uint8Array([0x45, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78]); // ISA_BROADCAST + shared data
    ...
    if (colonyType === 3) {
        ...
        console.log(`❌ Colony not formed yet. Crystal type: ${colonyType} (expected 3)`);
    ...
    for (let i = 10; i < 16; i++) {
        ...
        STATE_MATRIX.setLogic(i, new Uint8Array([0xAA, 0xAA, 0xAA, 0xAA, 0x01, 0x02, 0x03, 0x04]));
    }
    for (let i = 20; i < 23; i++) {
        STATE_MATRIX.setId(i, BigInt(i + 1));
        ...
        STATE_MATRIX.setLogic(i, new Uint8Array([0xBB, 0xBB, 0xBB, 0xBB, 0x05, 0x06, 0x07, 0x08]));
    }
    ...
    if (result.colonySize >= 3) {
        ...
        console.log(`   Colony size: ${result.colonySize} atoms (vs 3 in rival Colony B)`);
