...
try {
    ...
    assert(lastLlmInput.hormoneRegime === "aggressive_bloom", `Passed correct regime to LLM: ${lastLlmInput.hormoneRegime}`);
    ...
    LLM_SYNAPSE.generateSpeciesTaxonomy = () => Promise.reject(new Error("LLM Fail"));
    ...
    const aggrEntry = (AKASHA_CODEX as any)._getSpeciesIndex().find((e: any) => e.genome === aggressiveGenome);
    ...
    assert(["Agressor", "Praedo", "Bellum"].includes(aggrGenus), `Aggressive regime uses correct genus: ${aggrGenus}`);
    ...
    const repairEntry = (AKASHA_CODEX as any)._getSpeciesIndex().find((e: any) => e.genome === repairGenome);
    // Genus for repair_surge: ["Sano", "Medicus", "Regen"]
    const repairGenus = repairEntry.latinName.split(" ")[0];
    assert(["Sano", "Medicus", "Regen"].includes(repairGenus), `Repair regime uses correct genus: ${repairGenus}`);
    ...
    assert(latestChronicle.type === "species_discovery", "Latest chronicle is species_discovery");
    assert(latestChronicle.title.includes("(Repair Surge)"), `Chronicle title mentions regime: ${latestChronicle.title}`);
    assert(latestChronicle.body.includes("Repair Surge conditions"), "Chronicle body mentions context");
...
console.log(`\n${"─".repeat(50)}`);
console.log(
  `🧪 HORMONE SPECIES EVOLUTION CONTRACT: ${failed === 0 ? "✅ PASS" : "❌ FAIL"} (${passed}/${
    passed + failed
  })`,
);
