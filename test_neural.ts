...
function baseSynaptic(weights: [number, number, number, number] = [0,0,0,0]): Int32Array {
...
function baseState(overrides: Record<string, unknown> = {}) {
    return {
        ...
        energy: 80, resonance: 300, // > HEBB_THRESHOLD (200)
...
Deno.test("Era 52: ISA.HEBB emits hebbRequest when atom resonance > 200", () => {
    ...
    assert(result.hebbRequest, "HEBB should emit hebbRequest when resonance > 200");
...
Deno.test("Era 52: ISA.HEBB suppressed when resonance ≤ 200", () => {
    ...
    assertEquals(result.hebbRequest, undefined, "HEBB should NOT fire when resonance ≤ 200");
});
...
Deno.test("Era 52: ISA.HEBB suppressed when bond slot is empty (targetIdx=0)", () => {
    ...
    assertEquals(result.hebbRequest, undefined, "HEBB should not fire without bonded neighbour");
});
...
Deno.test("Era 52: ISA.FIRE emits intent level 18 with correct weight and amplitude", () => {
    ...
    assertEquals(fireIntent.value.weight, 200, "Weight should be 200 (from synapticStack)");
    assert(result.energyDelta < 0, "FIRE costs energy proportional to weight×amplitude");
});
...
Deno.test("Era 52: ISA.FIRE skips signal when synaptic weight ≤ 10", () => {
    ...
    const fireIntent = result.intent.find((it: any) => it.level === 18);
    assertEquals(fireIntent, undefined, "FIRE should not propagate when weight ≤ 10");
});
...
Deno.test("Era 52: ISA.SENSE type 0x08 reads synaptic weight into register", () => {
    ...
    assertEquals(ctx2[2 + 1], 180, "Register at index p2%8=1 should hold weight 180 from slot p2%4=1");
});
