...
Deno.test("Mainnet Bootstrap: Hub Discovery and WebSocket Fallback", async () => {
  ...
  try {
    ...
    assertEquals(nexusB.connectedPeers.size > 0, true, "Node B should have discovered peers via Hub");
    assertEquals(nexusC.connectedPeers.size > 0, true, "Node C should have discovered peers via Hub");
    ...
    const timeout = new Promise((_, reject) => {
        timerId = setTimeout(() => reject(new Error("Timeout waiting for Atom")), 2000);
    });
    await Promise.race([receiptPromise, timeout]);
    if (timerId) clearTimeout(timerId);

    // Asserts: Structural Payload consistency checked
    assertEquals(receivedPayload !== null, true, "Payload should be populated upon Network Receipt over Hub-discovered link");
    assertEquals(receivedPayload!.length, 256, "Should decode exactly 256 payload bytes from transit");
    assertEquals(receivedPayload![0], 0x88, "Head byte should map correctly");
    assertEquals(receivedPayload![255], 0xBB, "Tail byte should maintain identical block alignment across transmission");
