...
import { saveEpoch, loadEpoch } from "../CONTINUUM.ts";
import { LATTICE_MEMORY_END, WASM_MEMORY_PAGES } from "../OFFSETS.ts";
...
Deno.test("Continuum Binary Epoch Restoration Parity", async () => {
    ...
    const hashA = await hashMemArray(new Uint8Array(stateA.memory.buffer, 0, LATTICE_MEMORY_END));
    ...
    const hashB = await hashMemArray(new Uint8Array(stateB.memory.buffer, 0, LATTICE_MEMORY_END));

    assertEquals(hashA, hashB, `Chronosphere temporal drift detected at tick 150!`);
