...
import { STATE_SNAPSHOT_CanonizationEvent as CanonizationEvent, STATE_SNAPSHOT_LedgerEvent as LedgerEvent, STATE_SNAPSHOT_ViolationEvent as ViolationEvent } from "@omega";
...
export async function runTest() {
    ...
    try {
        ...
        const result = await CRYSTALLIZATION.enforcePostCrystal(104, "artifact_demo", {
            windowSize: 4,
            witness: "test"
        });
        ...
        if (result.rollbackTick !== 100) {
            throw new Error(`Expected rollback checkpoint 100, got ${result.rollbackTick}`);
        }
