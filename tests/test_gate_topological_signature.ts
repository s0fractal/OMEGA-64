...
import { STATE_SNAPSHOT_GateConfig as GateConfig, STATE_SNAPSHOT_DeltaProposal as DeltaProposal, STATE_SNAPSHOT_StateSnapshot as StateSnapshot } from "@omega";
import { TOPOLOGICAL_SIGNATURE__08_00_TOPOLOGICAL_SIGNATURE as TOPOLOGICAL_SIGNATURE, TOPOLOGICAL_SIGNATURE__08_00_TopologicalSignature as TopologicalSignature } from "@omega";
...
Deno.test("gate emits topological signature fields in ledger", async () => {
    ...
    try {
        ...
        const next = await GATE.process(genesis, [baseProposal(1, "state_1")], baseConfig());
        ...
        if (evt.projection_version !== TOPOLOGICAL_SIGNATURE.PROJECTION_VERSION) {
            throw new Error(`unexpected projection_version: ${evt.projection_version}`);
        }
        ...
        if (!verify.ok) {
            throw new Error(`topological signature verify failed: ${verify.reasons.join(",")}`);
        }
    } finally {
        try {
