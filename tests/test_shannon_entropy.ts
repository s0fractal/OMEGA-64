...
Deno.test("Stage 37: Information Thermodynamics (Noise Tax)", async () => {
  ...
  console.log("Executing physics loop for 50 ticks to accumulate metabolic costs...");
  for (let i = 0; i < 50; i++) {
    await PULSE.tick();
  }
  ...
  const pcA = Atomics.load(new Uint8Array(STATE_MATRIX.contexts.buffer), STATE_MATRIX.contexts.byteOffset + atomA * 64 + 32);
  const pcB = Atomics.load(new Uint8Array(STATE_MATRIX.contexts.buffer), STATE_MATRIX.contexts.byteOffset + atomB * 64 + 32);
  ...
  console.log(`Atom A - Remaining Energy: ${energyA}, PC: ${pcA}, EntropyCache: ${entropyA}`);
  console.log(`Atom B - Remaining Energy: ${energyB}, PC: ${pcB}, EntropyCache: ${entropyB}`);
