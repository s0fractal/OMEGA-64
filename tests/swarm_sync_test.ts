...
if (Deno.args[0] === "run-node") {
  ...
  async function runMaster() {
    ...
    const fastNode = new Deno.Command(Deno.execPath(), {
      args: ["run", "-A", "--unstable", import.meta.url, "run-node", "9010", "9011"],
      stdout: "piped",
      stderr: "inherit",
    }).spawn();

    const slowNode = new Deno.Command(Deno.execPath(), {
      args: ["run", "-A", "--unstable", import.meta.url, "run-node", "9011", "9010", "--delay"],
      stdout: "piped",
      stderr: "inherit",
    }).spawn();
    ...
    console.log(`📊 [RESULTS] Fast Node Tick: ${fastFinalTick} | Slow Node Tick: ${slowFinalTick}`);
    ...
    if (drift > 60) {
       console.error(`❌ [TEST FAILS] Drift (${drift}) exceeds 50! Fast node did not elastic yield.`);
       Deno.exit(1);
    }
