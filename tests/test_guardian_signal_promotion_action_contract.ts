...
const promote = evaluateGuardianSignalPromotionAction({
...
const hold = evaluateGuardianSignalPromotionAction({
  currentMode: "shadow-reduce",
  decision: {
    verdict: "hold",
...
const demote = evaluateGuardianSignalPromotionAction({
...
const hybridHold = evaluateGuardianSignalPromotionAction({
  currentMode: "hybrid-reduce",
  decision: {
    verdict: "promote",
    promotionReady: true,
...
if (failures.length > 0) {
  console.error("[guardian-signal-promotion-action] contract violated.");
...
console.log("[guardian-signal-promotion-action] contract guard passed.");
