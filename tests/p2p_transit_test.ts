...
Deno.test("P2P Transit: Nexus A routes Atom to Nexus B", async () => {
  ...
  try {
    ...
    assertEquals(receivedPayload !== null, true, "Payload should be populated upon Network Receipt");
    assertEquals(receivedPayload!.length, 256, "Should decode exactly 256 payload bytes from transit");
    assertEquals(receivedPayload![0], 0x42, "Head byte should map completely across the transit buffer");
    assertEquals(receivedPayload![255], 0xAA, "Tail byte should maintain identical block alignment across transmission");
