...
function test() {
  ...
  points.forEach(p => {
    ...
    console.log(`p: ${p.toString().padStart(6)} | log: ${comp.toFixed(2).padStart(8)} | exp: ${exp.toString().padStart(6)} | Diff: ${Math.abs(p - exp)}`);
  });
  ...
  states.forEach((s, i) => {
    ...
    console.log(`State ${i} [r: ${s.r.toString().padStart(6)}]: Potential: ${potential.toFixed(4)} | Decay: ${decay.toFixed(2)} | Pain: ${pain.toFixed(2)}`);
  });
}
