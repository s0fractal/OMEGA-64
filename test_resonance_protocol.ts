...
async function testResonance() {
    ...
    const neuralCoherenceView = new Int32Array(sharedBuffer, OFFSETS.NEURAL_COHERENCE_OFFSET, 1);
    ...
    console.log("   [STAGE 1] Low Coherence set: 50/255. Guardian should switch to ARCHITECT (Role 3).");
    ...
    if (role !== 3) throw new Error(`Guardian did not switch to ARCHITECT. got=${role}`);
    
    // 5. Test Autopoiesis
    console.log("   [STAGE 2] Testing Autopoiesis: Injecting high charge into Structure Grid.");
    ...
    console.log(`   [RESULT 2] Autopoietic Cell (10,10) Type: ${cellType} (Expected 1: WIRE)`);
    if (cellType !== 1) throw new Error(`Autopoiesis failed. got cellType=${cellType}`);

    // 6. Test Oracle Whisper Channel
    console.log("   [STAGE 3] Testing Oracle Whisper injection into MEMORY_GRID.");
    ...
    console.log(`   [RESULT 3] Whisper-seeded cells delta: ${afterSeeds - beforeSeeds} (Expected > 0)`);
    if (afterSeeds <= beforeSeeds) throw new Error("Oracle whisper did not seed MEMORY_GRID.");
