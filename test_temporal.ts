...
Deno.test("Era 54: PHASE_LIFE aged atom (200-399) emits FIRE signals when bonds+weights present", () => {
    ...
    const result = LAMBDA_VM.execute(logic, code, context, baseState(250, { bonds, synapticStack: synStack }));
    ...
    assertEquals(fires[0].value.amplitude, 150, "Teaching amplitude should be 150");
...
Deno.test("Era 54: PHASE_LIFE senescent atom (>=400) emits apoptosisRequest", () => {
    ...
    assertGreater(result.resonanceDelta, 0, "Final wisdom burst — resonance gain");
});
...
Deno.test("Era 54: ISA.SENSE type 0x0A reads age bucket into register", () => {
    ...
    for (const [age, expected, label] of testCases) {
        ...
        assertEquals(ctx[2], expected, `${label} (age=${age}) should be bucket ${expected}`);
    }
});
