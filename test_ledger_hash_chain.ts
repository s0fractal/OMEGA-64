...
Deno.test("ledger append emits chain fields and verifies green", async () => {
    await withTempLedger(async () => {
        ...
        const s2 = await GATE.process(genesis, [proposal("p1", 1, "state_1", 5)], baseConfig());
        await GATE.process(s2, [proposal("p2", 2, s2.state_hash, -2)], baseConfig());

        const raw = await Deno.readTextFile(LEDGER.STORAGE_PATH);
        const lines = raw.split("\n").filter((x) => x.trim().length > 0);
        if (lines.length !== 4) {
            throw new Error(`expected 4 lines (bridge+event x2), got ${lines.length}`);
        }
        ...
        for (const [i, line] of lines.entries()) {
            ...
            } else if (typeof evt.prev_event_hash !== "string" || evt.prev_event_hash.length !== 64) {
        ...
        if (!chain.ok) {
            throw new Error(`expected green ledger chain: ${chain.failures.join(",")}`);
        }
        if (chain.chainAnchoredEvents !== 4) {
            throw new Error(`expected 4 chain anchored events, got ${chain.chainAnchoredEvents}`);
        }
...
Deno.test("replay audit can fail-fast on ledger chain when enabled", async () => {
    await withTempLedger(async () => {
        ...
        await GATE.process(genesis, [proposal("p1", 1, "state_1", 5)], baseConfig());
        ...
        if (audit.replayGreen) {
            throw new Error("replay must fail when ledger chain check is enabled and chain is broken");
        }
        if (!audit.failures.some((f) => f.includes("ledger_chain:LEDGER_CHAIN_HASH_MISMATCH"))) {
            throw new Error(`missing expected ledger_chain failure: ${audit.failures.join(",")}`);
        }
        if (!audit.invariantReport.ledger_chain_checked || audit.invariantReport.ledger_chain_ok) {
            throw new Error("expected invariant report ledger_chain_checked=true and ledger_chain_ok=false");
        }
    });
});
