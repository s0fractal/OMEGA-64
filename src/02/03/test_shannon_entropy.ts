...
Deno.test("Stage 37: Information Thermodynamics (Noise Tax)", async () => {
  ...
  MX.setId(atomA, 100n);
  MX.setEnergy(atomA, 10000); // 10,000 Energy

  MX.setId(atomB, 101n);
  MX.setEnergy(atomB, 10000); // 10,000 Energy
  ...
  const energyB = MX.getEnergy(atomB);

  const pcA = Atomics.load(
    new Uint8Array(MX.contexts.buffer),
    MX.contexts.byteOffset + atomA * 64 + 32,
  );
  const pcB = Atomics.load(
    new Uint8Array(MX.contexts.buffer),
    MX.contexts.byteOffset + atomB * 64 + 32,
  );
  const entropyA = Atomics.load(MX.contexts, atomA * 16 + 15);
  const entropyB = Atomics.load(MX.contexts, atomB * 16 + 15);
