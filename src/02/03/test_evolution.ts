...
async function runTest() {
  ...
  console.log(
    "🌡️ Test 1: High-pressure mutation (low resonance = desperate)...",
  );

  const MUTATE_OPCODE = new Uint8Array([
    0x47,
    0x00,
    0xAA,
    0xBB,
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (
      logic[2] !== 0xAA || logic[3] !== 0xBB || logic[4] !== 0xCC ||
      logic[5] !== 0xDD
    ) {
      mutated++;
    }
  ...
  console.log(
    `✅ ${mutated}/5 low-resonance atoms mutated under pressure! 🌡️→🧬`,
  );
  ...
  const STABLE_OPCODE = new Uint8Array([
    0x47,
    0x11,
    0x22,
    0x33,
  ...
  for (let i = 10; i < 15; i++) {
    const logic = STATE_MATRIX.getLogic(i);
    if (logic[1] === 0x11 && logic[2] === 0x22 && logic[3] === 0x33) {
      unchanged++;
    }
  }
  ...
  STATE_MATRIX.setLogic(
    20,
    new Uint8Array([0x47, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07]),
  );
  ...
  if (cellType === CRYSTAL_MEME) {
    console.log(
      `✅ Fitness Propagation: crystal upgraded to CRYSTAL_MEME! (type ${cellType})`,
    );
    console.log(`   Fit genome is now broadcast to all atoms passing through.`);
  } else {
    const colonyType = cellType;
    console.log(
      `ℹ️  Crystal type at location: ${colonyType} (MEME upgrade may need more ticks or energy)`,
    );
  }
