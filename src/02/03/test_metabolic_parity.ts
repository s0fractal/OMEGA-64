...
import { ENERGY_OFFSET, LOGIC_OFFSET, MAX_ATOMS } from "/Users/s0fractal/OMEGA/src/_/mod.ts";
...
async function testMetabolicParity() {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    if (i < 5) {
      console.log(
        `   [DEBUG] Setting atom=${idx} key=${genomeKey} bytes=(${
          logicView[idx * 8]
        },${logicView[idx * 8 + 1]})`,
      );
    }
  ...
  await new Promise<void>((resolve) => {
    const handler = (e: MessageEvent) => {
      if (
        e.data.type === "METABOLISM_ACCUMULATE_DONE" &&
        e.data.pulseId === pulseId
      ) {
  ...
  await new Promise<void>((resolve) => {
    const handler = (e: MessageEvent) => {
      if (
        e.data.type === "METABOLISM_APPLY_DONE" &&
        e.data.pulseId === pulseId + 1
      ) {
        worker0.removeEventListener("message", handler);
        resolve();
      }
  ...
  for (let i = 0; i < MAX_ATOMS; i++) {
    ...
    if (wasmEnergy !== hostEnergy) {
      if (errors < 10) {
        console.error(
          `❌ Metabolism Mismatch @ atom=${i}: WASM=${wasmEnergy} HOST=${hostEnergy}`,
        );
      }
      errors++;
    }
  ...
  if (errors === 0) {
    ...
    console.log(
      `❌ [PARITY] Metabolic Homeostasis failed with ${errors} errors.`,
    );
    Deno.exit(1);
  }
}
