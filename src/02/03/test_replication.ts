...
async function runTest() {
  ...
  console.log(
    "Testing the full Birth → Life → Mutation → Death → Crystal cycle\n",
  );
  ...
  const REPLICATE_OPCODE = new Uint8Array([
    0x4A,
    0xDE,
    0xAD,
    0xBE,
  ...
  console.log(
    "🧬 Seeding 5 parent atoms with ISA_REPLICATE (0x4A) on crystal...",
  );
  for (let i = 0; i < 5; i++) {
  ...
  if (born > 0) {
    ...
    console.log(
      `   ✅ Self-replication confirmed — life has begun to replicate!`,
    );
  } else {
    ...
    console.log(
      `   Parent[0] energy: ${parentEnergy} (should be ~halved from 3000)`,
    );
    if (parentEnergy < 2500) {
      console.log(
        `✅ Energy tax confirmed — ISA_REPLICATE fired! (Spawn queue drained separately)`,
      );
    }
  }
  ...
  for (let i = 50; i < 55; i++) {
    ...
    STATE_MATRIX.setLogic(
      i,
      new Uint8Array([0x4A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
    );
  }
  ...
  if (illegalBirth <= 0) {
    console.log(
      `✅ Starved atoms did NOT replicate. Population delta: ${illegalBirth}`,
    );
  } else {
  ...
  console.log(
    `   👥 Final population: ${STATE_MATRIX.getActiveIndices().length}`,
  );
