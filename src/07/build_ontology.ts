...
const ALLOWED_TYPES = ["i32", "i64", "f32", "f64", "u8", "i16", "usize", "boolean", "bool", "void"];
...
${(node.regions || []).map((r, i, arr) => {
  ...
  if (node.level > 0) {
    const prevLevel = formatLevel(node.level - 1);
    if (node.vars && node.vars.length > 0) {
      asOut += `import { ${node.vars.join(", ")} } from "../${prevLevel}/mod";\n`;
    }
  }
  asOut += `\n`;
  ...
  if (node.type === "enum" || node.type === "constants" || node.type === "memory_layout" || node.type === "static_table") {
    ...
    let asArgStr = arr.map(a => `${a.name}: ${a.type === 'usize' ? 'usize' : (a.type === 'u8' ? 'u8' : 'i32')}`).join(", ");
    let asRet = node.returns === 'usize' ? 'usize' : 
                (node.returns === 'u8' ? 'u8' : 
                (node.returns === 'void' ? 'void' : 
                (node.returns === 'boolean' || node.returns === 'bool' ? 'bool' : 'i32')));
