...
interface OntologyNode {
  id: string;
  type?: "static_table" | "pure_fn" | "struct";
  description?: string;
  deps: string[];
...
for (const node of nodes.values()) {
  const dirPathTs = `${GEN_DIR_TS}/${formatLevel(node.level)}`;
  const dirPathRs = `${GEN_DIR_RS}/${formatLevel(node.level)}`;
...
for (let lvl = 0; lvl <= maxLevel; lvl++) {
  ...
  if (lvl > 0) {
    ...
    lvlTsOut += `import * as L${prevLvlStr} from "../${prevLvlStr}/mod";\nexport { L${prevLvlStr} };\n`;
  ...
  for (const node of nodesInLevel) {
    lvlTsOut += `export { ${node.id} } from "./${node.id}";\n`;
  ...
  mainTsOut += `export { ${nodesInLevel.map(n => n.id).join(", ")} } from "./${lvlStr}/mod";\n`;
