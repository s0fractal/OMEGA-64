import { renderCoreExport } from "@g";
...
const main = async () => {
  ...
  const exportedMarkdown = await Deno.readTextFile("src/08/OMEGA_RUNTIME.md");
  const persistedHeader = parseExportHeader(
    exportedMarkdown,
    "src/08/OMEGA_RUNTIME.md",
  );
  expect(
    persistedHeader.title.includes(`ERA ${era}`),
    `[export-provenance] 08/OMEGA_RUNTIME.md title does not include era=${era}`,
  );
  expect(
    persistedHeader.manifestHash === provenance.manifestSha256,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: manifest hash mismatch vs current render",
  );
  expect(
    persistedHeader.exportSetHash === provenance.exportSetSha256,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: export-set hash mismatch vs current render",
  );
  expect(
    persistedHeader.exportContentHash === provenance.exportContentSha256,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: export-content hash mismatch vs current render",
  );
  expect(
    persistedHeader.runtimeRootCount === runtimeRoots.length,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: runtime root count mismatch vs current render",
  );
  expect(
    persistedHeader.runtimeClosureCount === runtimeClosureFiles.length,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: runtime closure count mismatch vs current render",
  );
  expect(
    persistedHeader.nonRuntimeCodeCount === nonRuntimeCodeFiles.length,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: non-runtime code count mismatch vs current render",
  );
  expect(
    persistedHeader.runtimeSupportCount === runtimeSupportCodeFiles.length,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: runtime-support count mismatch vs current render",
  );
  expect(
    persistedHeader.experimentalCount === experimentalCodeFiles.length,
    "[export-provenance] 08/OMEGA_RUNTIME.md stale: experimental count mismatch vs current render",
  );
