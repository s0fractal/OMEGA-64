...
const ensureStringArray = (value: unknown, field: string): string[] => {
  if (!Array.isArray(value) || value.some((x) => typeof x !== "string")) {
    throw new Error(
      `[runtime-experimental-boundary] manifest field must be string[]: ${field}`,
    );
  }
...
const main = async () => {
  ...
  const experimentalFiles = await resolveAll(ensureStringArray(
  ...
  if (runtimeRoots.length === 0) {
    throw new Error(
      "[runtime-experimental-boundary] runtime_root_files cannot be empty",
    );
  }

  const experimentalSet = new Set(experimentalFiles);
  const visited = new Set<string>();
  const parent = new Map<string, string>();
  const queue = [...runtimeRoots];
  const directEdgesToExperimental: Array<{ from: string; to: string }> = [];
  ...
  if (missingRoots.length > 0) {
    throw new Error(
      `[runtime-experimental-boundary] runtime root missing on disk:\n${
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of specs) {
      ...
      if (experimentalSet.has(resolved)) {
        directEdgesToExperimental.push({ from: file, to: resolved });
      }
  ...
  const reachedExperimental = experimentalFiles.filter((f) => visited.has(f));
  if (reachedExperimental.length > 0) {
    const traces = reachedExperimental.map((f) => reconstructPath(parent, f));
    const traceLines = traces.map((chain) => `- ${chain.join(" -> ")}`);
    const directLines = directEdgesToExperimental.map((edge) =>
      `- ${edge.from} -> ${edge.to}`
    );
    throw new Error(
      `[runtime-experimental-boundary] runtime closure reached experimental modules.\nPaths:\n${
        traceLines.join("\n")
      }${
        directLines.length > 0
          ? `\nDirect runtime->experimental edges:\n${directLines.join("\n")}`
          : ""
      }`,
    );
  }
  ...
  console.log(
    `[runtime-experimental-boundary] guard passed. runtimeVisited=${visited.size} experimental=${experimentalFiles.length}`,
  );
};
