...
function mockStateMatrix(atoms: Array<{logic: number[], resonance: number, x: number, y: number, feedCount?: number, bonds?: number[], active: boolean}>) {
    return {
        getActiveIndices: () => atoms.map((a, i) => a.active ? i : -1).filter(i => i !== -1),
        ...
        setQuarantine: (i: number, level: number) => { atoms[i].quarantine = level; },
...
Deno.test("Era 62: GATE penalizes variants with resonance lower than baseline", () => {
    ...
    const atoms = [
        ...
        {logic: [0x20,0x20,0x20,0x20, 0x20,2,2,2], resonance: 5000, x: 0, y: 0, active: true},
        {logic: [0x20,0x20,0x20,0x20, 0x20,2,2,2], resonance: 5000, x: 0, y: 0, active: true},
        {logic: [0x20,0x20,0x20,0x20, 0x20,2,2,2], resonance: 5000, x: 0, y: 0, active: true},
    ];
    ...
    assertEquals(atoms[2].quarantine, 2, "Toxic variant suppressed (Quarantine 2)");
});
...
Deno.test("Era 62: GATE promotes high-resonance variants to trustedSignatures", () => {
    ...
    assert(GATE.trustedSignatures.has(plasmidHex), "Beneficial plasmid promoted to Canon");
});
...
Deno.test("Era 62: Trusted signatures ignore antigen checks", () => {
    ...
    const atoms = [
        {logic: [0x20,0x20,0x20,0x20, 0x20,0x20,0x20,0x20], resonance: 10000, x: 0, y: 0, active: true}
    ];
