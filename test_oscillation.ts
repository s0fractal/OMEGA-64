...
async function runTest() {
    ...
    const ampView = new Uint32Array(STATE_MATRIX.buffer, MEMORY_GRID_BASE + center * 8, 1);
    const amplitude = ampView[0];

    if (centerType === CRYSTAL_OSCILLATOR) {
        console.log(`✅ CRYSTAL_OSCILLATOR formed! (type ${centerType})`);
    } else {
        console.log(`ℹ️  Center crystal type: ${centerType} (oscillator may form with more ticks)`);
    }
    console.log(`   📶 Center signal: ${centerSig}`);
    console.log(`   🧠 Standing-wave amplitude: ${amplitude} (accumulated resonance memory)`);

    if (amplitude > 0) {
        console.log(`✅ Memory trace confirmed! Oscillation encoded in memoryGrid.`);
    }
    ...
    if (isolatedType !== CRYSTAL_OSCILLATOR) {
        console.log(`✅ Isolated crystal remained type ${isolatedType} — no false oscillation. 🔇`);
    } else {
    ...
    // ── FINAL SUMMARY ─────────────────────────────────────────────────────
    console.log(`\n📊 Oscillation Summary:`);
    console.log(`   🌊 Center type: ${centerType} (${centerType === CRYSTAL_OSCILLATOR ? 'OSCILLATOR' : 'standard'})`);
