...
async function runTest() {
    ...
    for (const [cx, cy] of junctions) {
        const cells = [
            cy * 140 + cx,
            (cy-1)*140 + cx, (cy+1)*140 + cx,
            cy*140 + (cx-1), cy*140 + (cx+1),
        ];
    ...
    for (const [cx, cy] of junctions) {
        ...
        const ampView = new Uint32Array(STATE_MATRIX.buffer, MEMORY_BASE + cellIdx * 8, 1);
        totalAmplitude += ampView[0];
        console.log(`   🧠 Junction (${cx},${cy}) amplitude: ${ampView[0]}`);
    }

    const coherence = oscillatorCount > 0 ? Math.min(2000, Math.floor(totalAmplitude / oscillatorCount)) : 0;
    ...
    const coherenceView = new Int32Array(STATE_MATRIX.buffer, NEURAL_COHERENCE_ADDR, 1);
    ...
    for (let i = 30; i < 35; i++) {
        ...
        STATE_MATRIX.setLogic(i, new Uint8Array([0x49, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]));
    }
    ...
    if (resonanceAfter > resonanceBefore) {
        console.log(`✅ ISA_SENSE boosted resonance: ${resonanceBefore} → ${resonanceAfter} (+${resonanceAfter - resonanceBefore})`);
    } else {
        console.log(`ℹ️  Resonance: ${resonanceBefore} → ${resonanceAfter} (boost from coherence field)`);
    }
