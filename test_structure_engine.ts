...
function runTest() {
    ...
    if (regrownType !== STRUCTURE.WIRE) {
        throw new Error(`[TEST] Expected autopoietic regrowth into WIRE, got type=${regrownType}`);
    }
    if (regrownCharge < 64) {
        throw new Error(`[TEST] Expected seeded autopoietic charge >= 64, got ${regrownCharge}`);
    }
    ...
    if (collapsedType !== STRUCTURE.VOID || collapsedCharge !== 0) {
        throw new Error(`[TEST] Expected isolated structure to decay into VOID/0, got type=${collapsedType}, charge=${collapsedCharge}`);
    }

    console.log("✅ [TEST] STRUCTURE_ENGINE propagation/autopoiesis/entropy verified.");
}
