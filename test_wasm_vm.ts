...
async function runWasmExpansionTests() {
    ...
    const vmState = { x: 0, y: 0, nutrients: new Int32Array(1), marketPool: new Int32Array(1), energy: 100, resonance: 0, bonds: new Uint32Array(4) };

    console.log("--- Executing Cycle 0 (CALL) ---");
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if (context[0] !== 8) { console.error("❌ CALL failed. PC is:", context[0]); Deno.exit(1); }
    if (context[18] !== 1 || context[10] !== 1) { console.error("❌ CALL stack push failed."); Deno.exit(1); }

    console.log("--- Executing Cycle 1 (LOAD) ---");
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if (context[0] !== 9) { console.error("❌ LOAD failed PC inc."); Deno.exit(1); }

    console.log("--- Executing Cycle 2 (MUL) ---");
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if (context[2 + 1] !== 25) { console.error("❌ MUL failed. R1 is:", context[2 + 1]); Deno.exit(1); }

    console.log("--- Executing Cycle 3 (CMP) ---");
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if ((context[1] & 0x01) !== 1) { console.error("❌ CMP failed to set Zero Flag. Flags:", context[1]); Deno.exit(1); }

    console.log("--- Executing Cycle 4 (JZ) ---");
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if (context[0] !== 13) { console.error("❌ JZ failed to branch. PC is:", context[0]); Deno.exit(1); }

    console.log("--- Executing Cycle 5 (RET) ---");
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if (context[0] !== 1) { console.error("❌ RET failed to return to PC 1. PC is:", context[0]); Deno.exit(1); }
    if (context[18] !== 0) { console.error("❌ RET failed to decrement SP."); Deno.exit(1); }

    console.log("--- Executing Cycle 6 (JMP) ---");
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if (context[0] !== 15) { console.error("❌ JMP failed to branch. PC is:", context[0]); Deno.exit(1); }
    ...
    if (logicAfterStore[3] !== 0xAA) { console.error("❌ Wasm STORE to LOGIC failed."); Deno.exit(1); }
    
    LAMBDA_VM.execute(STATE_MATRIX.getLogic(1), STATE_MATRIX.getCode(1), context, vmState);
    if (context[2 + 1] !== 0xAA) { console.error("❌ Wasm LOAD from LOGIC failed."); Deno.exit(1); }

    console.log("✅ [TEST] Wasm Viral Base Genome mutation active!");
    console.log("🎉 [VERIFIED] Era 43 Phase 2 is complete. AssemblyScript Kernel achieves structural coherence.");
}
