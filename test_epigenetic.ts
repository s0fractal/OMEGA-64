...
Deno.test("Era 56: MITOSIS child inherits parent synapticStack[0..2]", () => {
    ...
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 0), 80, "Child inherits w0=80");
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 1), 120, "Child inherits w1=120");
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 2), 200, "Child inherits w2=200");
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 3), 0, "Child FIRE tally resets to 0");
...
Deno.test("Era 56: MEIOSIS child averages both parents' synapticStack[0..2]", () => {
    ...
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 0), 80, "Child w0 = avg(100,60)=80");
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 1), 80, "Child w1 = avg(60,100)=80");
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 2), 50, "Child w2 = avg(20,80)=50");
    assertEquals(STATE_MATRIX.getSynapticValue(childIdx, 3), 0, "FIRE tally resets");
...
Deno.test("Era 56: ISA.INHERIT moves synapticStack[p1] toward hiveMemory reference", () => {
    ...
    const state = baseState({ synapticStack: synStack, hiveMemory: makeHiveMemory(100) });
    ...
    assert(result.modifiedSynaptic !== undefined, "INHERIT should emit modifiedSynaptic");
...
Deno.test("Era 56: ISA.INHERIT moves weight DOWN when current > reference", () => {
    ...
    const state = baseState({ synapticStack: synStack, hiveMemory: makeHiveMemory(50) });
    LAMBDA_VM.execute(logic, code, context, state);

    assertEquals(synStack[1], 197, "Weight should decrease from 200 toward 50 by -3");
});
...
Deno.test("Era 56: ISA.INHERIT uses amplitude=1 when p2=0", () => {
    ...
    const state = baseState({ synapticStack: synStack, hiveMemory: makeHiveMemory(80) });
    LAMBDA_VM.execute(logic, code, context, state);

    assertEquals(synStack[2], 41, "Weight should move by +1 with default amplitude");
});
...
Deno.test("Era 56: ISA.SENSE type 0x0C reads imprint age into register", () => {
    ...
    LAMBDA_VM.execute(logic, code, context, baseState({ hiveMemory: hm, age: 10 }));
...
Deno.test("Era 56: Epigenetic noise stays clamped to [0, 255]", () => {
    ...
    for (let base of [0, 127, 255]) {
        ...
        assert(result >= 0 && result <= 255, `Weight ${base} + noise must stay in [0,255]`);
    }
});
