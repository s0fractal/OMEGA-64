...
Deno.test("Stage 30: Networked Cognition (P2P Syscalls)", async () => {
  ...
  assertEquals(msgTypeAfterRead, 0, "Inbox should be cleared after reading (msgType=0)");
  assertEquals(payloadAfterRead, 0, "Inbox should be cleared after reading (payload=0)");
  ...
  assertEquals(senderEnergy < 980, true, `Sender should be taxed for sending MSG. Energy: ${senderEnergy}`);
