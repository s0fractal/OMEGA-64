...
Deno.test({
  ...
  code[0] = RISC.OP_ADD; code[1] = 1; code[2] = 1;
  code[3] = RISC.OP_JMP; code[4] = 0;

  STATE_MATRIX.seedAtom(atomIdx, 100n, 100, 100, initialEnergy, 500, new Uint8Array(8), code);
  ...
  const pc = Atomics.load(new Int32Array(STATE_MATRIX.buffer, OFFSETS.CONTEXT_OFFSET + 32, 14000 * 16), atomIdx);

  const energyAfter1Tick = STATE_MATRIX.getEnergy(atomIdx);
  console.log(`[TEST] Energy after 1 tick: ${energyAfter1Tick}`);
  assertEquals(energyAfter1Tick < initialEnergy, true, "Energy should explicitly be depleted by execution cost");
  ...
  console.log(`[TEST] PC: ${STATE_MATRIX.getPC(atomIdx)}, Final Energy: ${finalEnergy}`);
  
  assertEquals(finalEnergy === 0, true, "Energy should be completely depleted resulting in OOG halt");
