...
function interference(waves: {r: number, phi: number, amp: number}[]): {r: number, phi: number, amp: number} {
    let sum_x = 0.0;
    let sum_y = 0.0;
    ...
    return {r: Math.round(res_r), phi: Math.round(res_phi_u16), amp: Math.round(res_amp)};
}
...
if (res3.amp > 14000 && res3.amp < 14300) console.log("✅ SUCCESS: Vector sum correct.");
