...
Deno.test("Stage 32: Network Mitosis (P2P Genetic Replication)", async () => {
  ...
  assertEquals(childGenome[2], SYS.REPLICATE, "Child genome must match Parent genome");
  
  // 2. Child PC is reset to 0
  assertEquals(childPc, 0, "Child PC should be reset to 0 upon replication");
  
  // 3. Child ID assigned
  assertEquals(childId, 3n, "Child must be assigned an ID upon instantiation (targetIdx + 1)");
  
  // 4. Energy transfer validation
  // Cost: Syscall base = 100, Replication spark = 50. Total parent cost ~ 150 + metabolic. 
  assertEquals(parentEnergy < 850, true, "Parent energy must be billed for Mitosis (100 cost + 50 spark)");
  ...
  assertEquals(childEnergy > 10, true, "Child must receive the metabolic spark and be alive");
