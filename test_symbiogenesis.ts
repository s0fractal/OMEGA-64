...
const parasiteLogic = new Uint8Array([0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00]);
...
console.log(`   [TEST] Quarantine state: ${q === 1 ? "FLAGGED" : q === 2 ? "SUPPRESSED" : "CLEAN"}`);

console.log("   [TEST] Step 2: Boosting resonance and energy (Proving worth)...");
...
console.log(`   [TEST] Quarantine state after success: ${q === 0 ? "CLEAN (Learned)" : "STILL FLAGGED"}`);

const logicStr = Array.from(parasiteLogic).map(b => b.toString(16).padStart(2, '0')).join('');
...
console.log("   [TEST] Step 3: Verified structureGrid is populated for Shelter.");
