// i.L99.core.O_STREAM_SUMMARY_RUN.ts
// @noncanonical
...
const usage = (): string =>
  [
    "Usage:",
    "  deno run -A i.L99.core.O_STREAM_SUMMARY_RUN.ts --input <stream.jsonl> [--pretty]",
  ].join("\n");

export const O_STREAM_SUMMARY_RUN = async (args: string[]): Promise<void> => {
  ...
  if (!parsed.input) {
    throw new Error("O_STREAM_SUMMARY_RUN: --input is required");
  }

  const proposals = await O_STREAM_READ(parsed.input);
  const summary = O_STREAM_SUMMARY(proposals);
...
if (import.meta.main) {
  O_STREAM_SUMMARY_RUN(Deno.args);
}
