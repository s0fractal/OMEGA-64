...
function baseSynaptic(w0=0, w1=0, w2=0, tally=0): Int32Array {
    ...
    arr[0] = w0; arr[1] = w1; arr[2] = w2; arr[3] = tally;
    return arr;
}
...
Deno.test("Era 53: ISA.ATTUNE emits roleRequest when signal tally >= threshold", () => {
    ...
    assert(result.roleRequest, "ATTUNE should emit roleRequest when tally >= threshold");
    assertEquals(result.roleRequest!.role, 1, "Dominant weight w0 → role 1 (Producer)");
...
Deno.test("Era 53: ISA.ATTUNE suppressed when tally < threshold", () => {
    ...
    assertEquals(result.roleRequest, undefined, "ATTUNE should not fire when tally < threshold");
});
...
Deno.test("Era 53: ATTUNE respects explicit p2 role override", () => {
    ...
    assertEquals(result.roleRequest!.role, 7, "p2=7 should override auto-derived role");
});
...
Deno.test("Era 53: ATTUNE uses custom threshold from p1", () => {
    ...
    assert(result.roleRequest, "ATTUNE should fire when tally=5 >= custom threshold=5");
...
Deno.test("Era 53: FIRE signal increments target synapticStack[3] tally", () => {
    ...
    assertEquals(STATE_MATRIX.getSynapticValue(idxA, 3), 1, "Signal tally should be 1 after one FIRE hit");
    ...
    assertEquals(STATE_MATRIX.getSynapticValue(idxA, 3), 21, "After 21 FIREs, tally should be 21");
