...
const LEDGER_RUNTIME_PATH = "DAEMON_PLASMID_LEDGER_RUNTIME.ts";
const LEDGER_PERSISTENCE_PATH = "DAEMON_PLASMID_LEDGER_PERSISTENCE.ts";
...
const main = async () => {
  ...
  requireSnippet(
    ...
    LEDGER_RUNTIME_PATH,
    "Daemon plasmid ledger runtime must provide apply path",
    violations,
  );
  requireSnippet(
    ledgerRuntime,
    ...
    LEDGER_RUNTIME_PATH,
    "Daemon plasmid ledger runtime must provide rollback path",
    violations,
  );
  requireSnippet(
    ledgerPersistence,
    ...
    "Daemon plasmid ledger persistence must append and compact durable history",
    violations,
  );
  requireSnippet(
    ledgerPersistence,
    ...
    LEDGER_PERSISTENCE_PATH,
    "Daemon plasmid ledger persistence must replay records into runtime state",
    violations,
  );
  requireSnippet(
    ledgerPersistence,
    ...
    LEDGER_PERSISTENCE_PATH,
    "Daemon plasmid ledger persistence must expose canonical snapshot path",
    violations,
  );
  ...
  requireSnippet(
    ...
    POLICY_PATH,
    "Daemon ingress policy must expose runtime sync helper for max plasmid charge",
    violations,
  );
  requireSnippet(
    policy,
    ...
    POLICY_PATH,
    "Daemon ingress policy must expose reset helper for max plasmid charge",
    violations,
  );
  ...
  requireSnippet(
    ...
    SYSTEM_START_PATH,
    "System runtime must route daemon max plasmid charge through ledger apply path",
    violations,
  );
  requireSnippet(
    system,
    ...
    SYSTEM_START_PATH,
    "System runtime must route daemon max plasmid charge through ledger rollback path",
    violations,
  );
  requireSnippet(
    system,
    "syncDaemonPlasmidPolicyLedgerHydration",
    SYSTEM_START_PATH,
  ...
  requireSnippet(
    ...
    SYSTEM_START_PATH,
    "System runtime must expose ledger-owned daemon plasmid-charge summary to observers",
    violations,
  );
  requireSnippet(
    system,
    ...
    SYSTEM_START_PATH,
    "System runtime must expose daemon plasmid-charge persistence summary to observers",
    violations,
  );
  requireSnippet(
    system,
  ...
  requireSnippet(
    ...
    SYSTEM_START_PATH,
    "System daemon ingress must enforce dynamic plasmid policy limit",
    violations,
  );
  ...
  if (violations.length > 0) {
    console.error("[daemon-plasmid-ledger-path] contract violated.");
  ...
  console.log("[daemon-plasmid-ledger-path] contract guard passed.");
};
