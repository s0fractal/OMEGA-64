...
import { MATRIX_ENGINE, CRYSTAL_STANDARD, CRYSTAL_MEME } from "./MATRIX_ENGINE.ts";
...
async function runTest() {
    ...
    const spawnAtom = (idx: number, id: bigint, x: number, y: number, energy: number, resonance: number, opcode: number) => {
    ...
    console.log("💎 Test 1: ISA_READ_MATRIX — atom attunes to local crystal field...");
    ...
    if (resAfter > resBefore) {
        console.log(`✅ Atom attuned! Resonance: ${resBefore} → ${resAfter} (+${resAfter - resBefore})`);
    } else {
    ...
    if (signalAfter > signalBefore) {
        console.log(`✅ Crystal charged! Signal: ${signalBefore} → ${signalAfter} (+${signalAfter - signalBefore})`);
        console.log(`   Atom resonance: 800 → ${resonanceAfter}`);
    } else {
        console.log(`❌ Injection failed. Signal: ${signalBefore} → ${signalAfter}`);
    }
    ...
    if (absorbed > 0) {
        console.log(`✅ Memetic Transfer: ${absorbed}/20 atoms absorbed the regent genome! 🧬`);
    } else {
        console.log(`ℹ️  No transfers in 5 ticks — stochastic (12.5%/tick). Expected in full simulation.`);
    }
