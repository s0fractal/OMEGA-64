    ...
    try {
        ...
        await PROPOSAL_ENVELOPE_INDEX.appendFromLedgerEvent(
            mkEvent("evt_2", 11, "s10", "s11", [
                {
                    proposal_id: "p2",
                    envelope_hash: "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
                }
            ])
        );
        ...
        if (!verify.ok) {
            throw new Error(`expected index chain ok, failures: ${verify.failures.join(",")}`);
        }
        const recent = await PROPOSAL_ENVELOPE_INDEX.getRecentEnvelopeHashes(11, 11);
        if (!recent.has("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")) {
            throw new Error("expected recent envelope hash for tick 11");
        }
        if (recent.has("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")) {
...
Deno.test("proposal envelope index chain detects tamper", async () => {
    ...
    try {
        ...
        first.envelope_hash = "dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd";
        ...
        if (!verify.failures.some((f: string) => f.includes("ENVELOPE_INDEX_RECORD_HASH_MISMATCH"))) {
