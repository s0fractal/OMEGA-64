...
async function runTest() {
  ...
  console.log(`-> Initial State:
       Atom ${parentIdx} (PRODUCER) at (50, 50), Energy: ${
    STATE_MATRIX.getEnergy(parentIdx)
  }
       Atom ${childIdx} (NEUTRAL) at (55, 55), Energy: ${
    STATE_MATRIX.getEnergy(childIdx)
  }`);
  ...
  const finalParentPos = [
    STATE_MATRIX.getX(parentIdx),
    STATE_MATRIX.getY(parentIdx),
  ];
  const finalChildPos = [
    STATE_MATRIX.getX(childIdx),
    STATE_MATRIX.getY(childIdx),
  ];
  ...
  if (finalChildPos[0] === 55 && finalChildPos[1] === 55) {
    console.warn(
      "⚠️ WARNING: Atom did not move. (Maybe velocity was too low or dampened)",
    );
  } else {
    console.log(
      "✅ SUCCESS: Physics (Chemotaxis/Movement) verified in WASM kernel.",
    );
  }

  const withinBounds = (x: number, y: number) =>
    x >= 0 && x <= 1399 && y >= 0 && y <= 799;
  if (
    !withinBounds(finalParentPos[0], finalParentPos[1]) ||
    !withinBounds(finalChildPos[0], finalChildPos[1])
  ) {
    console.error(
      "❌ FAILED: Position escaped world bounds (0..1399, 0..799).",
    );
    success = false;
  } else {
