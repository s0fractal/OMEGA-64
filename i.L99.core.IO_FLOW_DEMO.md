...
Suggested commands:
- Append a proposal:
  deno task o:append -- --input OMEGA_O_STREAM.jsonl --proposal <proposal.json>
- Run IO_FLOW health write:
  deno task io:health:write -- --input <input.json> --output UI/health_io.json --pretty --safe-window
- Start UI (reads health.json or health_io.json):
  SAFE_WINDOW=1 HEALTH_JSON=1 MODE=io INPUT=<input.json> ./ui_health.sh
