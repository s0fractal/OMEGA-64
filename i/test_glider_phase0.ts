...
async function runTest() {
    ...
    console.log(`✅ Next Hash: ${nextState.state_hash} (Should remain genesis_hash_000 in dry-run)`);
    ...
    for await (const event of LEDGER.readAll()) {
        ...
        if (event.tick === 100) {
            if (event.accepted_proposals.includes("prop_A") && 
                event.rejected_proposals.some(r => r.reason === REJECTION.TICK_MISMATCH)) {
