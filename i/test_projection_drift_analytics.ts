...
const proposal = (id: string, tick: number, baseHash: string, delta: Array<{ level: number; value: number }>): DeltaProposal => ({
    proposal_id: id,
    tick,
...
Deno.test("projection drift analytics produces deterministic timeline", async () => {
    await withTempLedger(async () => {
        ...
        const s2 = await GATE.process(
            genesis,
            [proposal("p1", 1, "state_1", [{ level: 32, value: 40 }, { level: 13, value: 5 }])],
            baseConfig()
        );
        await GATE.process(
            { tick: 2, state_i16: s2.state_i16, state_hash: s2.state_hash },
            [proposal("p2", 2, s2.state_hash, [{ level: 32, value: -15 }, { level: 5, value: 9 }])],
            baseConfig()
        );
        ...
        if (!r1.ok) {
            throw new Error(`expected ok report, got failures: ${r1.failures.join(",")}`);
        }
...
Deno.test("projection drift analytics fails when replay audit is not green", async () => {
    await withTempLedger(async () => {
        ...
        if (report.ok) {
            throw new Error("expected report.ok=false when replay audit is not green");
        }
        if (!report.failures.some((x) => x === "REPLAY_AUDIT_NOT_GREEN")) {
            throw new Error(`expected REPLAY_AUDIT_NOT_GREEN, got: ${report.failures.join(",")}`);
        }
    });
});
