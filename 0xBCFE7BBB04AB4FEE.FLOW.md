---
eigenvalue: "0x4BCFE7BB04AB4FEE"
vector: 05.05.01
symbol: "FLOW"
desc: "Functional flow (Pipe). (f => g => x => g(f(x)))"
---

<div align="center">
<!-- ∇ HOLOGRAM START ∇ -->
<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <radialGradient id="grad_4BCFE7BB04AB4FEE" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="hsl(112, 20%, 15%)" />
      <stop offset="100%" stop-color="#090909" />
    </radialGradient>
    <filter id="glow_4BCFE7BB04AB4FEE" x="-20%" y="-20%" width="140%" height="140%">
      <feGaussianBlur stdDeviation="4" result="blur" />
      <feComposite in="SourceGraphic" in2="blur" operator="over" />
    </filter>
  </defs>
  
  <rect width="200" height="200" fill="url(#grad_4BCFE7BB04AB4FEE)" rx="24"/>
  
  <circle cx="100" cy="100" r="85" stroke="hsl(112, 30%, 30%)" stroke-width="1" fill="none" stroke-dasharray="2 6"/>
  
  <g>
    <animateTransform attributeName="transform" type="rotate" from="270 100 100" to="630 100 100" dur="25s" repeatCount="indefinite" />
    
    <polygon points="100.0,41.0 159.0,100.0 100.0,159.0 41.0,100.0" fill="none" stroke="hsl(292, 60%, 40%)" stroke-width="1.5" opacity="0.6"/>
    <polygon points="100.0,65.0 135.0,100.0 100.0,135.0 65.0,100.0" fill="none" stroke="hsl(112, 80%, 60%)" stroke-width="2" filter="url(#glow_4BCFE7BB04AB4FEE)"/>
    
    <circle cx="100" cy="100" r="35" stroke="hsl(112, 60%, 50%)" stroke-width="0.5" fill="none" opacity="0.5"/>
  </g>
  
  <circle cx="100" cy="100" r="3" fill="hsl(292, 80%, 70%)" filter="url(#glow_4BCFE7BB04AB4FEE)"/>
  
  <text x="100" y="105" fill="hsl(112, 70%, 80%)" font-family="monospace" font-size="10" text-anchor="middle" letter-spacing="1" opacity="0.9">0x4BCF</text>
  <text x="100" y="190" fill="#777" font-family="monospace" font-size="9" text-anchor="middle" letter-spacing="2">FLOW</text>
</svg>
<!-- Δ HOLOGRAM END Δ -->
</div>


## GREEN (G)

Functional FLOW. A composition operator that pipes the output of function `f`
into the input of function `g`. `FLOW f g x = g(f(x))`.

## BLUE (B)

```typescript
/**
 * FLOW: Composable pipeline
 */
export const ATOM = () => (f: any) => (g: any) => (x: any) => g(f(x));
```
