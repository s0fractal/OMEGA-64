...
async function runTest() {
  ...
  const instantiated = await WebAssembly.instantiate(wasmCode, {
    index: { 
      trace_atom: (idx: number, op: number, gx: number, gy: number, t: number) => {} 
    },
    env: {
      memory: wasmMemory,
      trace_atom: (idx: number, op: number, gx: number, gy: number, t: number) => {},
  ...
  // 1. Initialize Grids
  const attentionField = new Float32Array(sharedBuffer, OFFSETS.ATTENTION_FIELD_OFFSET, 140 * 80);
  ...
  const signalGridU8 = new Uint8Array(sharedBuffer, OFFSETS.SIGNAL_GRID_OFFSET, 140 * 80 * 9); // Wait, signalGrid is Int32Array but it's 9 bytes per cell? 
  ...
  for (let b = 0; b < 4; b++) memoryGrid[200 * 8 + b] = [0xDE, 0xAD, 0xBE, 0xEF][b];

  // Viral/Signal at idx 300: Logic [1,2,3,4,0,0,0,0] | Intensity 200
  const viralIdx = 300 * 9;
  const viralU8 = new Uint8Array(sharedBuffer, OFFSETS.SIGNAL_GRID_OFFSET, 140 * 80 * 9);
  ...
  for (let i = 0; i < attentionField.length; i++) {
    if (Math.abs(attentionField[i] - hostAttention[i]) > 0.000001) {
       console.error(`Attention mismatch at ${i}: WASM=${attentionField[i]}, Host=${hostAttention[i]}`);
  ...
  for (let i = 0; i < structureGrid.length; i++) {
    if (structureGrid[i] !== hostStructure[i]) {
        console.error(`Structure mismatch at ${i}: WASM=${structureGrid[i]}, Host=${hostStructure[i]}`);
  ...
  console.log(`📡 Viral Parity: ${(viralMatchRatio * 100).toFixed(2)}% (${viralMatches}/${hostViral.length})`);
  
  if (viralMatchRatio < 1.0) {
      console.warn("⚠️ Viral diffusion mismatch detected. Investigating PRNG alignment...");
