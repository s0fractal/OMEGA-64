...
const main = () => {
  ...
  expect(ids.size === HORMONE_BUFFER_CATALOG.length, "[hormone_buffer] ids must be unique");

  const indices = new Set(HORMONE_BUFFER_CATALOG.map((spec) => spec.index));
  expect(indices.size === HORMONE_BUFFER_CATALOG.length, "[hormone_buffer] indices must be unique");

  for (const spec of HORMONE_BUFFER_CATALOG) {
    expect(spec.min <= spec.defaultValue, `[hormone_buffer] default below min for ${spec.id}`);
    expect(spec.defaultValue <= spec.max, `[hormone_buffer] default above max for ${spec.id}`);
    expect(spec.notes.trim().length > 0, `[hormone_buffer] notes missing for ${spec.id}`);
    expect(spec.sourcePath.trim().length > 0, `[hormone_buffer] sourcePath missing for ${spec.id}`);
  }
