...
function makeQuorum(cellCounts: Record<number, number[]>): Int32Array {
    // 140*80 cells * 8 roles each
    const arr = new Int32Array(new SharedArrayBuffer(140 * 80 * 8 * 4));
    // gx=50, gy=40 → cell index = 40*140+50 = 5650
    const cellIdx = 40 * 140 + 50;
    for (const [role, counts] of Object.entries(cellCounts)) {
        counts.forEach((count, r) => { arr[cellIdx * 8 + r] = count; });
        break; // only one entry needed
    }
    return arr;
}

function baseState(role: number, quorumData: Int32Array, overrides: Record<string, unknown> = {}) {
    return {
        x: 500, y: 400, // maps to gx=50, gy=40
...
Deno.test("Era 55: ISA.QUORUM emits quorumRequest when same-role count >= threshold", () => {
    ...
    const qData = new Int32Array(new SharedArrayBuffer(140 * 80 * 8 * 4));
    const cellIdx = 40 * 140 + 50;
    ...
    assert(result.quorumRequest, "QUORUM should emit quorumRequest when count >= threshold");
    assertEquals(result.quorumRequest!.collectiveType, 0, "p2=0 → resonance cascade");
    assertEquals(result.quorumRequest!.quorumCount, 8, "Quorum count should be 8");
});
...
Deno.test("Era 55: ISA.QUORUM suppressed when same-role count < threshold", () => {
    ...
    const result = LAMBDA_VM.execute(logic, code, context, baseState(1, qData));
    assertEquals(result.quorumRequest, undefined, "QUORUM should not fire below threshold");
});
...
Deno.test("Era 55: QUORUM type 0 gives resonance bonus proportional to count", () => {
    ...
    assertGreater(result.resonanceDelta, 0, "Resonance cascade gives positive resonanceDelta");
...
Deno.test("Era 55: QUORUM type 2 grants resonance for role lock", () => {
    ...
    assertEquals(result.quorumRequest!.collectiveType, 2, "collectiveType should be 2");
    assertGreater(result.resonanceDelta, 0, "Role lock grants resonance stability bonus");
});
...
Deno.test("Era 55: ISA.QUORUM default threshold is 5 when p1=0", () => {
    ...
    assert(result.quorumRequest, "Default threshold=5: count=5 should trigger QUORUM");
});
...
Deno.test("Era 55: STATE_MATRIX.getRole reads correctly set role", () => {
    ...
    assertEquals(STATE_MATRIX.getRole(idx), 5, "getRole should return the set role 5");
