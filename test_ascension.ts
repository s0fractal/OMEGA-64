...
Deno.test("Era 64: ISA.ASCEND emits request if energy>=5000 and resonance>=500", () => {
    ...
    const result = LAMBDA_VM.execute(logic, code, context, baseState({ energy: 5000, resonance: 550 }));
...
Deno.test("Era 64: ISA.ASCEND fails if conditions not met", () => {
    ...
    let result = LAMBDA_VM.execute(logic, code, context, baseState({ energy: 4999, resonance: 600 }));
    assert(!result.ascendRequest, "Fails if energy < 5000");

    result = LAMBDA_VM.execute(logic, code, context, baseState({ energy: 6000, resonance: 499 }));
...
Deno.test("Era 64: PULSE_WORKER transforms atom into Crystal structure", () => {
    ...
    assertEquals(((resultingStructure >> 8) & 0xFF), 255, "Structure density is 255 (Max)");
