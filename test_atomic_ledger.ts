...
import { ATOMIC_LEDGER } from "./ATOMIC_LEDGER.ts";
...
Deno.test("Stage 33: Binary Event Ledger (SYS_EMIT)", async () => {
  ...
  scriptA[0] = RISC.OP_SET; scriptA[1] = 0; scriptA[2] = SYS.EMIT;
  scriptA[3] = RISC.OP_SET; scriptA[4] = 1; scriptA[5] = 9; // R1
  scriptA[6] = RISC.OP_SET; scriptA[7] = 2; scriptA[8] = 120; // R2
  scriptA[9] = RISC.OP_SYSCALL; scriptA[10] = 0;
  STATE_MATRIX.setInstructions(atomA, scriptA);

  // Atom B emits [123, 200]
  const scriptB = new Uint8Array(64);
  scriptB[0] = RISC.OP_SET; scriptB[1] = 0; scriptB[2] = SYS.EMIT;
  scriptB[3] = RISC.OP_SET; scriptB[4] = 1; scriptB[5] = 123; // R1
  scriptB[6] = RISC.OP_SET; scriptB[7] = 2; scriptB[8] = 200; // R2
  scriptB[9] = RISC.OP_SYSCALL; scriptB[10] = 0;
  ...
  LOGGER.info(`Event 1: Atom ${events[0].atomIdx} at Tick ${events[0].tick} -> [${events[0].r1}, ${events[0].r2}]`);
  LOGGER.info(`Event 2: Atom ${events[1].atomIdx} at Tick ${events[1].tick} -> [${events[1].r1}, ${events[1].r2}]`);

  // We don't guarantee the exact order between 10 and 11, but both events must be in there.
  const hasEventA = events.some(e => e.atomIdx === atomA && e.r1 === 9 && e.r2 === 120 && e.tick === 42);
  const hasEventB = events.some(e => e.atomIdx === atomB && e.r1 === 123 && e.r2 === 200 && e.tick === 42);
  ...
  assertEquals(binaryDump.byteLength, EXPECTED_SIZE, "Binary dump should match exactly the head + 64K event payload");
