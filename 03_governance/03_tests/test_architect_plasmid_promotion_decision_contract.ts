...
const promote = evaluateArchitectPlasmidPromotionDecision({
  promotion: {
    latestReady: true,
    readyRatio: 0.75,
...
const holdPromotion = evaluateArchitectPlasmidPromotionDecision({
  promotion: {
    latestReady: false,
    readyRatio: 0.25,
...
const holdHealth = evaluateArchitectPlasmidPromotionDecision({
  promotion: {
    latestReady: true,
    readyRatio: 0.8,
    recommendedMode: "hybrid-reduce",
...
const holdDaemon = evaluateArchitectPlasmidPromotionDecision({
  promotion: {
    latestReady: true,
    readyRatio: 0.7,
    recommendedMode: "hybrid-reduce",
...
if (failures.length > 0) {
  console.error("[architect-plasmid-promotion-decision] contract violated.");
...
console.log("[architect-plasmid-promotion-decision] contract guard passed.");
