...
async function test_coherence_field() {
    console.log("🧬 [TEST] Starting Dedicated Global Coherence Field Verification...");
    let failed = false;
    ...
    for (let i = 1; i <= 100; i++) {
        STATE_MATRIX.seedAtom(i, BigInt(i), 50, 50, 1000, 300, undefined, signalGenome);
    }
    ...
    console.log(`   [TEST] Global Coherence after 100 atoms signaled: ${coherence}`);
    
    if (coherence >= 100) {
        console.log("   [PASS] Coherence aggregation verified.");
    } else {
        console.error(`   [FAIL] Coherence aggregation failed. Got ${coherence}, expected >= 100.`);
        failed = true;
    }

    // 3. Verify Phase Synchronization (using high volume signaling)
    console.log("   [TEST] Testing Phase Synchronization (Harmonic Alignment)...");
    
    // Seed 600 atoms to cross the 500 threshold
    for (let i = 1; i <= 600; i++) {
        STATE_MATRIX.seedAtom(i, BigInt(i), 50, 50, 2000, 300, undefined, signalGenome);
    }

    const syncAtomIdx = 700;
    STATE_MATRIX.seedAtom(syncAtomIdx, 700n, 60, 60, 1000, 0, undefined, new Uint8Array(64));
    ...
    console.log(`   [TEST] Atom Phase after sync tick: ${phaseAfter} (Expected: >10)`);
    
    if (phaseAfter > 10) {
        console.log("   [PASS] Phase synchronization verified.");
    } else {
        console.error(`   [FAIL] Phase synchronization failed. Phase remained at ${phaseAfter}. Coherence was ${globalCoherenceFinal}`);
        failed = true;
    }
