  ...
  ignite: async (options: {
    ...
    const cycle = async () => {
      ...
      activeAgents.forEach((agent) => {
        ...
        if (result.action === "SPAWN") {
          const child: MyceliumAgent = {
            ...
            wave: {
              ...
              center: result.newAgent.wave.center +
        ...
        if (
          result.action.startsWith("Moved") || result.action === "PhaseShift"
        ) {
          const proposal = MYCELIUM.toProposal(
            result.newAgent,
            result.action,
            currentState,
          );
      ...
      if (proposals.length > 0) {
        ...
        console.log(
          `[TICK ${currentState.tick}] 🛡️ GATE: Bridge=${output.bridge_mode} | Accepted=${proposals.length}`,
        );
      } else {
        currentState.tick++;
      }
      ...
      if (t % 10 === 0) {
        ...
        for (const peer of PEER.knownPeers.values()) {
          if (peer.tick > currentState.tick + 2) {
            console.log(
              `⚡ SWARM: Detected advanced peer [${peer.id}] (Tick ${peer.tick} > ${currentState.tick})`,
            );
            isSyncing = true;
            try {
