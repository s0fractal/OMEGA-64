...
export const GLIDER_LITE_RUN = async (args: string[]): Promise<void> => {
  ...
  if (!raw.trim()) {
    throw new Error("GLIDER_LITE_RUN: empty input");
  }
  ...
  const toProposals = (proposals: DeltaProposal[]): DeltaProposal[] =>
  ...
  const input: GateRunnerTickInput = {
    state: toState(json.state),
    proposals: toProposals(json.proposals ?? []),
  ...
  const output = await GLIDER_LITE(input);
