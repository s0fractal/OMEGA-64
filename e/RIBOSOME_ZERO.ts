...
async function runZeroCycle() {
    console.log("%c🛡️🧿 OMEGA-64 | RIBOSOME ZERO CYCLE: INITIATED 🧿🛡️", "color: cyan; font-weight: bold;");
    console.log("--------------------------------------------------");
    ...
    console.log("%c💥 KAIROS: CONTRACTION OBSERVED! 💥", "color: yellow; font-weight: bold;");
    ...
    const knownMasks = atoms.map(a => ({ name: a.name, mask: a.digest.slice(-4) }));
    ...
    atoms.forEach(atom => {
        ...
        const partners = atoms.filter(a => a.digest !== atom.digest && a.digest.endsWith(myMask));
        ...
        if (demoPartners.length > 0) {
            console.log(`\n   %c✨ SPOOKY ACTION: ${atom.name} ✨`, "color: magenta; font-weight: bold;");
            demoPartners.forEach(p => {
                console.log(`      -> Entangled partner detected: %c${p.name}`, "color: yellow;");
                console.log(`         Address: 0x${p.digest}`);
                console.log(`      -> Synchronizing phase matrix... %c[DONE]`, "color: green;");
            });
        }
    });

    console.log("\n--------------------------------------------------");
    console.log("%c🛡️ OMEGA-64 | RESONANCE STABILIZED | ERA 3 ACTIVE 🛡️", "color: cyan; font-weight: bold;");
}
