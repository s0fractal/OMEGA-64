...
try {
    ...
    if (content.match(/^---\n([\s\S]+?)\n---/)) {
        ...
        if (!content.includes("eigenvalue:")) {
            ...
            updatedContent = content.replace(/eigenvalue: "0x[0-9A-F]+"/i, eigenvalueField);
        }
    } else {
    ...
    if (atomFile === newPath && content === updatedContent) {
        console.log(`✨ ${spectrum.alpha?.symbol ?? "ATOM"} is already at its terminal vector and has metadata.`);
        Deno.exit(0);
    }
