---
eigenvalue: '0x53300000DC2EB5E9'
vector: 05.00.00
symbol: TENSOR
desc: Tensor constructor. VECTOR(dims)(values)
energy: 95
ex:
  - '0x95DA9A3CDC2EB5E9'
---

<div align="center">
<!-- ∇ HOLOGRAM START ∇ -->
<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <radialGradient id="grad_53300000DC2EB5E9" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="hsl(255, 20%, 15%)" />
      <stop offset="100%" stop-color="#090909" />
    </radialGradient>
    <filter id="glow_53300000DC2EB5E9" x="-20%" y="-20%" width="140%" height="140%">
      <feGaussianBlur stdDeviation="4" result="blur" />
      <feComposite in="SourceGraphic" in2="blur" operator="over" />
    </filter>
  </defs>
  
  <rect width="200" height="200" fill="url(#grad_53300000DC2EB5E9)" rx="24"/>
  
  <circle cx="100" cy="100" r="85" stroke="hsl(255, 30%, 30%)" stroke-width="1" fill="none" stroke-dasharray="2 6"/>
  
  <g>
    <animateTransform attributeName="transform" type="rotate" from="0 100 100" to="360 100 100" dur="16s" repeatCount="indefinite" />
    
    <polygon points="100.0,43.0 154.2,82.4 133.5,146.1 66.5,146.1 45.8,82.4" fill="none" stroke="hsl(75, 60%, 40%)" stroke-width="1.5" opacity="0.6"/>
    <polygon points="100.0,69.0 129.5,90.4 118.2,125.1 81.8,125.1 70.5,90.4" fill="none" stroke="hsl(255, 80%, 60%)" stroke-width="2" filter="url(#glow_53300000DC2EB5E9)"/>
    
    <circle cx="100" cy="100" r="31" stroke="hsl(255, 60%, 50%)" stroke-width="0.5" fill="none" opacity="0.5"/>
  </g>
  
  <circle cx="100" cy="100" r="3" fill="hsl(75, 80%, 70%)" filter="url(#glow_53300000DC2EB5E9)"/>
  
  <text x="100" y="105" fill="hsl(255, 70%, 80%)" font-family="monospace" font-size="10" text-anchor="middle" letter-spacing="1" opacity="0.9">0x5330</text>
  <text x="100" y="190" fill="#777" font-family="monospace" font-size="9" text-anchor="middle" letter-spacing="2">TENSOR</text>
</svg>
<!-- Δ HOLOGRAM END Δ -->
</div>


## GREEN (G)

N-dimensional Tensor. Constructs a complex data structure by pairing a `dims`
vector (shape) with a `values` vector. Uses the foundational Level 5 `VECTOR`
for aggregation.

## BLUE (B)

```typescript
/**
 * TENSOR: Multi-dimensional aggregate
 */
export const ATOM =
  ({ siblings: { VECTOR } }) => (dims: any) => (values: any) =>
    VECTOR(dims)(values);
```
