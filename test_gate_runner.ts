...
Deno.test("gate runner routes through replay/invariant contexts", async () => {
    ...
    try {
        ...
        const s2 = await GATE_RUNNER.step({
            state: s1.nextState,
            proposals: [proposal("p_canon_green", s1.nextState.tick, s1.nextState.state_hash, 1, 11, "CANON")],
            config: baseConfig(),
        ...
        for await (const evt of LEDGER.readAllRaw()) {
            if ("event_type" in evt && evt.event_type === "BRIDGE_MODE_EVENT") bridgeEvents++;
            if (!("event_type" in evt)) ledgerEvents++;
        }
        if (bridgeEvents !== 2) {
            throw new Error(`expected 2 BRIDGE_MODE_EVENT entries, got ${bridgeEvents}`);
        }
        if (ledgerEvents !== 2) {
