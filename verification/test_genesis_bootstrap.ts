...
import { GLYPH_SPECS, glyphSpecById } from "../reduction_core/GlyphIR64.ts";
...
function testChromaticMapping() {
  ...
  console.assert(s?.rgb?.[0] === 255 && s?.rgb?.[1] === 255 && s?.rgb?.[2] === 255, "S Glyph should be White");
...
function testGenesisPrograms() {
  ...
  console.assert(GENESIS_PROGRAMS["guardian_base"] !== undefined, "guardian_base missing");
  console.assert(GENESIS_PROGRAMS["architect_base"].length >= 4, "architect_base too short");
