import { STATE_MATRIX, RISC, STRUCTURE } from "../STATE_MATRIX.ts";
...
const runCapture = async (): Promise<CapturePayload> => {
  ...
  const trace_atom = (idx: number, op: number, p1: number, p2: number, p3: number) => {
    console.log(`[KERNEL_TRACE] idx=${idx} op=0x${op.toString(16)} p1=${p1} p2=${p2} p3=${p3}`);
  };
  ...
  const roleScript = Uint8Array.of(
    RISC.OP_SET, 0, STATE_MATRIX.ROLE_GUARDIAN,
    RISC.OP_RESOLVE, 0, 2,
    RISC.OP_NOP
  );
  ...
  const bankScript = Uint8Array.of(
    RISC.OP_RESOLVE, 1, 100,
    RISC.OP_NOP
  );

  STATE_MATRIX.seedAtom(0, 202n, bx, by, 1000, 100, undefined, bankScript);
  // Set regs[8] (gene0) to control pool slot
  const instructions = new Uint8Array(STATE_MATRIX.buffer, OFFSETS.INSTRUCTIONS_OFFSET, 100000 * 64);
  ...
  const context = new Int32Array(STATE_MATRIX.buffer, OFFSETS.CONTEXT_OFFSET, 100000 * 16);
  ...
  const poolView = new Int32Array(STATE_MATRIX.buffer, OFFSETS.HIVE_ENERGY_POOL_OFFSET, 256);
  let totalPool = 0;
  for(let i=0; i<256; i++) {
