...
async function main() {
    ...
    try {
        ...
        if (!status.fallbackActivated) throw new Error("[TEST] Startup fallback was not activated.");
        if (runtimeWorkerCount !== 1) throw new Error(`[TEST] Expected runtime workers=1, got=${runtimeWorkerCount}`);
        if (faults.length !== 1) throw new Error(`[TEST] Expected fault stats for exactly one worker, got=${faults.length}`);
