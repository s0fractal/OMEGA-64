...
async function runTest() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        // Set ID to activate
        new BigUint64Array(STATE_MATRIX.buffer, OFFSETS.IDS_OFFSET, 100)[i] = BigInt(i + 1);
    }
    ...
    console.log(`📊 Ascended: ${ascendedCount} | Remaining: ${activeIndices.length}`);
    if (ascendedCount === 64) {
        console.log("✅ SUCCESS: Ascension was throttled at 64.");
    } else {
        console.error(`❌ ERROR: Ascension Throttling failed (Expected 64, got ${ascendedCount})`);
    }
