...
import { betPoolInt, PREDICTION_MARKET } from "./PREDICTION_MARKET.ts";
...
const losingLogic = new Uint8Array([
  0x00,
  0x00,
  0x00,
  0x00,
...
console.log(
  `   [TEST] Pool: ${poolEnergy}, Winner Energy: 100, Loser Energy: 100`,
);
...
console.log(
  `   [TEST] NEW Pool: ${newPool}, NEW Winner Energy: ${
    newWinnerEnergy.toFixed(2)
  }, NEW Loser Energy: ${newLoserEnergy.toFixed(2)}`,
);

const dividend = poolEnergy * 0.1;
if (
  newWinnerEnergy > 100 && newLoserEnergy === 100 &&
  newPool === (poolEnergy - dividend)
) {
