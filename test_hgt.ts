...
function baseState(viralGrid: Uint8Array, logicVals: number[] = [], x = 500, y = 400) {
    const logic = new Uint8Array(8);
    for (let j=0; j<Math.min(8, logicVals.length); j++) logic[j] = logicVals[j];
...
Deno.test("Era 60: ISA.SECRETE_PLASMID emits Request with own logic and intensity", () => {
    ...
    assertEquals(result.secretePlasmidRequest!.intensity, 150, "Intensity should be 150");
    assertEquals(result.secretePlasmidRequest!.logic[5], 255, "Should copy own logic byte 5");
...
Deno.test("Era 60: PULSE_WORKER applying secretePlasmidRequest deposits to viralGrid", () => {
    ...
    const req = { logic: new Uint8Array([11, 22, 33, 44, 55, 66, 77, 88]), intensity: 75 };
...
Deno.test("Era 60: ISA.INCORPORATE_PLASMID emits Request when intensity > p1", () => {
    ...
    const result = LAMBDA_VM.execute(logic, code, context, baseState(vGrid, [0,0,0,0,0,0,0,0]));
    
    assert(result.incorporatePlasmidRequest, "Should emit incorporatePlasmidRequest");
    assertEquals(result.incorporatePlasmidRequest!.logic[0], 99, "Should read plasmid logic byte 0");
...
Deno.test("Era 60: ISA.INCORPORATE_PLASMID ignores plasmid if intensity <= p1", () => {
    ...
    assert(!result.incorporatePlasmidRequest, "Should NOT emit incorporatePlasmidRequest if intensity low");
});
...
Deno.test("Era 60: PULSE_WORKER applying incorporatePlasmidRequest overwrites logic and reduces intensity", () => {
    ...
    for (let j = 0; j < 8; j++) {
        ...
        if (globalLogicArray) Atomics.store(globalLogicArray, idx * 8 + j, req.logic[j]);
    }
    ...
    // Verify
    assertEquals(vGrid[cellBase + 8], 90, "Cell viral intensity decremented by 10");
    if (globalLogicArray) {
        assertEquals(globalLogicArray[idx * 8 + 0], 8, "Atom logic byte 0 overwritten with plasmid");
    }
    if (rolesArray) {
