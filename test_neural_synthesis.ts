...
async function test_neural_synthesis() {
  ...
  console.log(
    `   [TEST] Cell (2,2) charge after OP_SIGNAL: ${charge} (Expected: >=170)`,
  );
  if (charge >= 170) {
  ...
  archGenome[0] = 0xA7;
  archGenome[1] = 0;
  archGenome[2] = 3; // OP_ROLE SET, 3 (ARCHITECT)
  archGenome[3] = 0xA8;
  archGenome[4] = 1;
  archGenome[5] = 0; // OP_BUILD 1 (WIRE), state 0
  ...
  console.log(
    `   [TEST] Cell (3,3) structure type: ${builtType} (Expected: 1)`,
  );
  if (builtType === 1) {
  ...
  syncGenome[1] = 6;
  syncGenome[2] = 0;
  syncGenome[3] = 0; // OP_COLLECTIVE mode 6
  ...
  const contextView = new Uint32Array(
    STATE_MATRIX.buffer,
  ...
  console.log(
    `   [TEST] Atom 4 PC after Collective Sync: ${atom4PC_real} (Expected: 4)`,
  );
  if (atom4PC_real === 4) {
