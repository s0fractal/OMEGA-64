...
const testLogic = new Uint8Array([0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22]);
...
for (let b = 0; b < 8; b++) {
    if (Atomics.load(viralGrid, viralIdx + b) !== testLogic[b]) logicMatches = false;
}

console.log(`   [TEST] Leaked Intensity: ${leakedIntensity} (Target: ${50-29})`); // decay reduces 30 to 29
