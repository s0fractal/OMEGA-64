...
const seedScenario = (seed: number, replicators: number, architects: number): void => {
    STATE_MATRIX.clear();
...
const gridProbe = (grid: Int32Array, x0: number, y0: number, w: number, h: number): number[] => {
    const out: number[] = [];
...
const runCapture = async (): Promise<CapturePayload> => {
    ...
    const seed = parseEnvInt("OMEGA_SPAWN_DETERMINISM_SEED", DEFAULT_SEED, 1, 0x7fffffff);
    const ticks = parseEnvInt("OMEGA_SPAWN_DETERMINISM_TICKS", DEFAULT_TICKS, 1, 200);
    const replicators = parseEnvInt("OMEGA_SPAWN_DETERMINISM_REPLICATORS", DEFAULT_REPLICATORS, 2, 64);
    const architects = parseEnvInt("OMEGA_SPAWN_DETERMINISM_ARCHITECTS", DEFAULT_ARCHITECTS, 1, 32);
...
const runCaptureSubprocess = async (
    ...
    const cmd = new Deno.Command(Deno.execPath(), {
        args: ["run", "-A", "test_spawn_determinism.ts", "--capture"],
        env: {
            ...Deno.env.toObject(),
    ...
    if (res.code !== 0) {
        throw new Error(`[TEST] spawn capture failed (workers=${workerCount}, strict=${strict}).\n${merged}`);
    }
    ...
    if (!line) {
        throw new Error(`[TEST] spawn capture marker missing (workers=${workerCount}, strict=${strict}).\n${merged}`);
    }
...
async function main() {
    ...
    console.log(`🧪 [TEST] Spawn determinism (strict 1 vs 4 workers) seed=${seed} ticks=${ticks} reps=${replicators} arch=${architects}`);
    const one = await runCaptureSubprocess(1, true, seed, ticks, replicators, architects);
    const four = await runCaptureSubprocess(4, true, seed, ticks, replicators, architects);
    ...
    if (one.hash !== four.hash) {
        console.error("❌ [TEST] Spawn determinism mismatch between worker topologies.");
        console.error(`   activeCount: ${one.snapshot.activeCount} vs ${four.snapshot.activeCount}`);
        console.error(`   tickCounter: ${one.snapshot.tickCounter} vs ${four.snapshot.tickCounter}`);
