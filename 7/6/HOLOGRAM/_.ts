...
export const HOLOGRAM = {
    ...
    digitalToOptical: (
        wave: HolographicProjection["wave"],
        chrono: HolographicProjection["chrono"]
    ): HolographicProjection => ({
        wave,
        chrono,
        intensity: Math.abs(wave.amplitude)
    }),

    opticalToDigital: (
        hologram: HolographicProjection
    ): { wave: HolographicProjection["wave"]; confidence: number } => ({
        wave: hologram.wave,
        confidence: Math.min(1, Math.abs(hologram.intensity) / 1000)
    }),

    interfere: (
        a: HolographicProjection,
        b: HolographicProjection
    ): HolographicProjection => ({
        wave: a.wave,
        chrono: a.chrono,
