import { GLYPH_BUFFER, GLYPH_KIND } from "./GLYPH_BUFFER.ts";
...
const main = () => {
  ...
  let amp = header >> 8;
  expect(amp === 150, `Constructive interference failed. Expected 150, got ${amp}`);
  
  // 2. Destructive Interference (+150 and -200 = -50)
  GLYPH_BUFFER.depositPheromone(50, 50, -200);
  header = STATE_MATRIX.getGlyphHeader(cell);
  amp = header >> 8;
  expect(amp === -50, `Destructive interference failed. Expected -50, got ${amp}`);
  ...
  expect(kind === GLYPH_KIND.NONE, `Annihilation kind failed. Expected NONE (0), got ${kind}`);
