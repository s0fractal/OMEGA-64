...
Deno.test("replay fails on local policy hash mismatch", async () => {
    await withTempLedger(async () => {
        ...
        if (!audit.failures.some((f) => f.includes("policy hash mismatch with local config"))) {
...
Deno.test("replay fails when policy changes without transition event", async () => {
    await withTempLedger(async () => {
        ...
        if (audit.replayGreen) {
            throw new Error("replay must fail on policy change without transition event");
        }
        if (!audit.failures.some((f) => f.includes("policy change without transition"))) {
            throw new Error(`missing expected failure, got: ${audit.failures.join(",")}`);
        }
    });
});
...
Deno.test("replay accepts policy transition when explicit event is present", async () => {
    await withTempLedger(async () => {
        ...
        if (transition.from_policy_version !== CRYSTALLIZATION_CONFIG.policyVersion) {
            throw new Error(`unexpected from_policy_version: ${transition.from_policy_version}`);
        }
        if (transition.from_policy_hash !== currentHash) {
            throw new Error(`unexpected from_policy_hash: ${transition.from_policy_hash}`);
        }
        ...
        if (!audit.replayGreen) {
            throw new Error(`replay should pass with explicit transition: ${audit.failures.join(",")}`);
        }
    });
});
