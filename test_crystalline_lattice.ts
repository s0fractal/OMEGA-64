...
async function test_crystalline_lattice() {
  console.log(
    "💎 [TEST] Starting Crystalline Lattice (WASM Grid) Verification...",
  );
  ...
  console.log(
    `   [TEST] End of wire charge at (5,0): ${endCharge} (Expected: > 0)`,
  );
  if (endCharge > 0) {
  ...
  console.log(
    `   [TEST] Role 0 quorum in cell (5,5): ${role0Count} (Expected: 10)`,
  );
  if (role0Count === 10) {
