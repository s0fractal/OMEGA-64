...
for (const [key, concept] of lattice) {
    ...
    if (missing.length > 0) {
        tensionCount++;
        tensions.push(`⚡ DISSONANCE in [${key}]: Missing dimensions [${missing.join(', ')}]. Vacuum detected.`);
    }

    // Check 2: Quantum Purity (TS files should be concise)
    if (concept.ts && concept.ts.lines > 50) {
        tensionCount++;
        tensions.push(`⚡ HEAVINESS in [${key}.ts]: ${concept.ts.lines} lines. Quantum Purity violation risk.`);
    }
    ...
    if (concept.ts && concept.rs) {
        const ratio = Math.max(concept.ts.size, concept.rs.size) / Math.min(concept.ts.size, concept.rs.size);
        if (ratio > 5.0) {
             tensionCount++;
             tensions.push(`⚡ ASYMMETRY in [${key}]: Mass Ratio ${ratio.toFixed(1)}. TS/RS balance disturbed.`);
        }
    }
}
...
if (tensionCount === 0) {
    ...
    if (tensions.length > 10) console.log(`...and ${tensions.length - 10} more faint signals.`);
