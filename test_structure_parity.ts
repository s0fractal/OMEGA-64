...
const setCell = (
    ...
    structureGrid[gridIndex(x, y)] = ((state & 0xFF) << 24) | ((charge & 0xFF) << 16) | ((density & 0xFF) << 8) | (type & 0xFF);
};
...
function seedScenario(structureGrid: Int32Array) {
    ...
    for (let x = 6; x <= 10; x++) setCell(structureGrid, x, 5, STRUCTURE.WIRE, 0, 0, 0);
...
async function runTest() {
    ...
    const structureGrid = new Int32Array(STATE_MATRIX.buffer, OFFSETS.STRUCTURE_GRID_OFFSET, GRID_SIZE);
    ...
    if (!tickStructure) {
        throw new Error("[TEST] WASM exports missing tick_structure_grid/tick_matrix.");
    }
    ...
    if (diffs.length > 0) {
        ...
        throw new Error(`[TEST] Found ${diffs.length}+ differing cells after ${ticks} ticks.`);
    }

    console.log(`✅ [TEST] STRUCTURE parity verified (${ticks} ticks, ${GRID_SIZE} cells).`);
}
