...
const runCapture = async (): Promise<CapturePayload> => {
  ...
  const seed = parseEnvInt(
    "OMEGA_STRUCTURE_INTENT_SEED",
    DEFAULT_SEED,
    1,
    0x7fffffff,
  );
  const ticks = parseEnvInt(
    "OMEGA_STRUCTURE_INTENT_TICKS",
    DEFAULT_TICKS,
    1,
    128,
  );
  const atomCount = parseEnvInt(
    "OMEGA_STRUCTURE_INTENT_ATOMS",
    DEFAULT_ATOMS,
    8,
    96,
  );
  ...
  const {
    ...
    conflictX,
    conflictY,
  } = seedConflictScenario(seed, atomCount);
...
const runCaptureSubprocess = async (
  ...
  if (res.code !== 0) {
    throw new Error(
      `[TEST] structure-intent capture failed (workers=${workerCount}, strict=${strict}).\n${merged}`,
    );
  }
  ...
  if (!line) {
    throw new Error(
      `[TEST] structure-intent capture marker missing (workers=${workerCount}, strict=${strict}).\n${merged}`,
    );
  }
...
const allSenseDetected = (payload: CapturePayload): boolean =>
  payload.snapshot.atoms.every((a) => a.senseReg === 1);
...
async function main() {
  ...
  console.log(
    `🧪 [TEST] Structure intent determinism | seed=${seed} ticks=${ticks} atoms=${atomCount}`,
  );
  ...
  if (one.hash !== four.hash) {
    ...
    throw new Error(
      [
        ...
        `hash(1w)=${one.hash}`,
        `hash(4w)=${four.hash}`,
        `conflict(1w)=type:${cellType(a)} state:${cellState(a)} charge:${
          cellCharge(a)
        }`,
        `conflict(4w)=type:${cellType(b)} state:${cellState(b)} charge:${
          cellCharge(b)
        }`,
      ].join("\n"),
    );
  ...
  if (!allSenseDetected(one) || !allSenseDetected(four)) {
    throw new Error(
      "[TEST] Structure intent visibility failed: OP_SENSE did not observe same-tick build intents.",
    );
  }

  const oneStrict = await runCaptureSubprocess(1, true, seed, ticks, atomCount);
  const fourStrict = await runCaptureSubprocess(
    4,
    true,
    seed,
    ticks,
  ...
  console.log(
    `✅ [TEST] Structure intent determinism + visibility verified. hash=${
      one.hash.slice(0, 12)
    }`,
  );
}
